 ...
 * whole sphere. Each ring vertex is displaced along the polar direction by an
 * OpenSimplex noise field sampled at the vertex's world-space position, so the
 * displacement is coherent across rings and drifts as the field animates.
 * Orientation random-walks over time and each ring is shaded from a circular
 * analogous palette that slowly spins across the stack.
 */
...
public:
  ...
  void init() override {
    ...
    register_param("Rings", &params.num_rings, 1.0f, 16.0f);
    register_param("Amplitude", &params.amplitude, 0.0f, 0.8f);
    register_param("Noise Scale", &params.noise_scale, 0.5f, 4.0f);
   ...
   * fragment's alpha.
   * @details Ring colatitudes are evenly spaced across the sphere. Each ring's
   * shift function samples the noise field at the undisplaced vertex position
   * in world space, so the pattern stays anchored in space while the random
   * walk spins the stack through it. The antipode remap mirrors azimuth for
   * radius > 1, so sampling runs in the same work frame as the plotted
   * vertices to keep the field seamless across the equator.
   */
  void draw_fn(Canvas &canvas, float opacity) {
    ...
    for (int i = 0; i < n_rings; ++i) {
      ...
      auto shift_fn = [this, &work, cos_eq, sin_eq](float t) {
        float angle = 2.0f * PI_F * t;
        Vector p = (work.v * cos_eq) +
                   ((work.u * cosf(angle)) + (work.w * sinf(angle))) * sin_eq;
        return params.amplitude *
               field_noise.GetNoise(p.x * params.noise_scale,
                                    p.y * params.noise_scale,
                                    p.z * params.noise_scale + field_time);
      };
      ...
      Plot::DistortedRing::draw<W, H>(filters, canvas, basis, radius, shift_fn,
                                      fragment_shader);
    }
  }
...
private:
  ...
  struct Params {
    ...
    float noise_scale = 1.5f; /**< Spatial frequency of the noise field over the unit sphere. */
