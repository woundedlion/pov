...
inline const Case *all_cases(int &n) {
  static const Case cases[] = {
      ...
      {"feedback_infinite_fade", case_feedback_infinite_fade, "styles.h",
       "(fade >= 0.0f && fade <= 1.0f) Feedback::Style::fade must be in "
       "[0, 1]"},
      {"gradient_no_stops", case_gradient_no_stops, "palette_sources.h",
      ...
      {"random_timer_inverted_range", case_random_timer_inverted_range,
       "timers.h", "(min >= 0 && min <= max) RandomTimer: invalid frame range"},
      ...
      {"sdf_flower_radius_over_hemisphere",
       case_sdf_flower_radius_over_hemisphere, "shapes.h",
      ...
      {"sdf_twist_zero_major_radius", case_sdf_twist_zero_major_radius,
       "volume.h", "(R > 0.0f) SDF Volume: radius must be positive"},
