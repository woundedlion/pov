...
struct ShadierBallWhiteBox {
  ...
  static Complex project(const Vector &v, Lens lens, float mix) {
    return SDB::project(v, lens, mix);
  }
  ...
  static Sample sample_pipeline(const Vector &v, Function function, Lens lens,
                                const TwinWaveState &wave, float pattern_freq,
                                float pole_fade, float lens_mix) {
    return SDB::sample_pipeline(v, {function, lens}, wave, pattern_freq,
                                pole_fade, lens_mix);
  }
...
inline void test_shadierball_lens_projection() {
  ...
  const Complex none = WB::project(v, WB::Lens::NONE, 0.7f);
  HS_EXPECT_EQ(none.re, direct.re);
  HS_EXPECT_EQ(none.im, direct.im);
  const Complex mix_zero = WB::project(v, WB::Lens::GLITCH, 0.0f);
  HS_EXPECT_EQ(mix_zero.re, direct.re);
  HS_EXPECT_EQ(mix_zero.im, direct.im);
  const Complex mix_full = WB::project(v, WB::Lens::GLITCH, 1.0f);
  HS_EXPECT_EQ(mix_full.re, lensed.re);
  HS_EXPECT_EQ(mix_full.im, lensed.im);

  const Complex before = WB::project(v, WB::Lens::GLITCH, 0.499f);
  const Complex middle = WB::project(v, WB::Lens::GLITCH, 0.5f);
  const Complex after = WB::project(v, WB::Lens::GLITCH, 0.501f);
  ...
  for (const Vector &d : dirs) {
    const float value =
        WB::sample_pipeline(d, WB::Function::TWIN_WAVE, WB::Lens::GLITCH, wave,
                            6.0f, 2.0f, 1.0f)
            .value;
