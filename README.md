...
7. [Core Subsystems](#7-core-subsystems)
   ...
   - [7.7 The Mesh System](#77-the-mesh-system-coremesh)
     ...
     - [Solids Library](#solids-library-solidsh)
...
Planar-warp **Speed** advances the stage's wrapped phase in cycles per frame. Affine Frame uses every non-neutral control as a motion amplitude on one closed orbit: Translation X/Y set its planar ellipse, Rotation and Shear oscillate with the quadrature phase, and Scale X/Y move logarithmically between reciprocal extrema. Speed adds no transform of its own. The affine transform itself joins exactly after one cycle, so its seam does not depend on the source pattern. Quadrature timing keeps combined controls from reversing in unison; a single bounded scalar control necessarily oscillates. Mirror Tile translates its mirror lattice by one local X cell, producing a seamless repeating scroll while its Y offset remains manual. Polar Chart advances only Angular Phase by one turn; Radial Phase remains manual. Wave Shear advances its wave, Vortex orbits its center, and the two projected-noise modes move through their periodic noise field.
