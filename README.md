...
├── .githooks/                  Staged pre-commit checks, post-commit size capture, and a reference-transaction guard keeping master fast-forward-only
...
for `setShaderChain`, `setShaderChainParameters` and `restoreFullConfigSnapshot`
...
Three further methods carry Shader's whole workbench configuration across a reload or into a segment worker, which the per-parameter stream above cannot. Replaying individual entries can walk through combinations the workbench refuses. None traps when the loaded effect is something else, so a caller may wire them unconditionally and hide the structural controls on the non-Shader answer.
