...
| `setClip(x0, x1, y0, y1)` → `bool` | Restrict rendering to a sub-rectangle (used by segment workers). `false` covers two rejections — no effect is set, or the bounds are out of range for the active resolution — so it is not on its own a bad-bounds signal. `true` does not mean the clip is in force: an effect reporting `needs_full_frame()` (§10.7) keeps the full-canvas clip *deliberately* and still returns `true`. A clip is dropped by any successful `setEffect()` or `setResolution()` and must be re-applied |
