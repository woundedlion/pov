...
- **Local pre-commit hook** ([`.githooks/pre-commit`](https://github.com/woundedlion/pov/blob/master/.githooks/pre-commit)) — a fast prefilter over staged files: clang-format for first-party C++, plus ruff/eslint when the matching language and installed tool are present. It does not build firmware or run the suite. Protected-branch `CI green` is authoritative, so local commits stay cheap without weakening what may merge.
