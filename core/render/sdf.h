...
struct FaceScratchBuffer {
  ...
  /**
   * @brief Per-edge constants for locating an edge's crossings of a scan row.
   * @details On row phi the edge's great circle n satisfies
   * cos(theta - psi) = k * cot(phi), so psi and k do not depend on the row.
   */
  struct RowEdge {
    float cos_psi, sin_psi; /**< Unit (n.x, n.z): the phase of the crossing. */
    float inv_r;            /**< Reciprocal of hypot(n.x, n.z). */
    float k;                /**< -n.y / r. */
    float psi;              /**< atan2(n.z, n.x); the crossing phase. */
    float arc_cos;          /**< dot(v1, v2); bounds the crossing to the arc. */
    float th_lo, th_hi;     /**< Azimuth extent of the arc; inverted disables
                                 the clip. */
    int16_t y_lo, y_hi;     /**< Rows the edge's arc can reach. */
  };
  std::array<RowEdge, MAX_VERTS> row_edges; /**< Per-edge row-crossing data. */
...
struct Face {
  ...
  using RowEdge = FaceScratchBuffer::RowEdge; /**< Per-edge row-crossing data. */
  std::span<RowEdge> row_edges; /**< Per-edge row-crossing constants. */
  bool row_spans_ok =
      false;            /**< Per-row spans usable for this face. */
  ...
  static constexpr int MAX_ROW_IV =
      32; /**< Unmerged intervals per row before the row falls back. */
  static constexpr int MAX_ROW_SPANS =
      4; /**< Spans per row; matches the scan's interval buffer. */
  static constexpr float ROW_SIN_EPS =
      1e-4f; /**< Below this sin(phi) the cot blows up. */
  static constexpr float MAX_PAD_COLS =
      24.0f; /**< Fringe wider than this reverts the row to the whole-face
                extent, which is cheaper to scan than the padded spans. */
  static constexpr int EDGE_ROW_SLACK =
      4; /**< Rows an edge stays active past its arc, covering FRINGE_PX of
            reach on a 2:1 equirectangular canvas. */
  static constexpr float FRINGE_PX =
      1.25f; /**< Fringe reach bounding a span, in equator columns. The scan
                paints d < one pixel and the gnomonic distance() reports at
                least the angular one, so a pixel's reach never exceeds a
                column; the rest is float and fast_acos slop. */
  static constexpr float ARC_POLE_GUARD =
      0.2f; /**< Polar angle within which an arc's azimuth extent stops
               bounding its fringe band. */
  ...
  HS_O3_FN Face(std::span<const Vector> vertices,
    ...
    { HS_PROFILE(face_row_spans);
      build_row_spans(scratch, h_virt, height);
    }
  }

  /**
   * @brief Precomputes each edge's row-crossing constants.
  ...
  void build_row_spans(FaceScratchBuffer &scratch, int h_virt, int height) {
    ...
    if (area2 == 0.0f)
      return;
    winding = (area2 > 0.0f) ? 1.0f : -1.0f;

    row_edges = std::span<RowEdge>(scratch.row_edges.data(), count);
    const float margin = thickness + BOUNDS_MARGIN;
    for (int i = 0; i < count; ++i) {
      const Vector &n = edge_normals[i];
      const float r_sq = n.x * n.x + n.z * n.z;
      // A great circle through both poles meets a latitude circle degenerately.
      if (r_sq < 1e-12f)
        return;

      RowEdge &e = row_edges[i];
      const float inv_r = 1.0f / sqrtf(r_sq);
      e.inv_r = inv_r;
      e.cos_psi = n.x * inv_r;
      e.sin_psi = n.z * inv_r;
      e.k = -n.y * inv_r;
      e.psi = fast_atan2(e.sin_psi, e.cos_psi);
      e.arc_cos = dot(verts_3d[i], verts_3d[i + 1]);

      float min_phi = fast_acos(hs::clamp(verts_3d[i].y, -1.0f, 1.0f));
      float max_phi = fast_acos(hs::clamp(verts_3d[i + 1].y, -1.0f, 1.0f));
      if (min_phi > max_phi) {
        const float t = min_phi;
        min_phi = max_phi;
        max_phi = t;
      }
      refine_phi_from_arc_extremum(n, verts_3d[i], verts_3d[i + 1], min_phi,
                                   max_phi);
      set_arc_azimuth_extent(e, verts_3d[i], verts_3d[i + 1], min_phi, max_phi);
      const Bounds rows = phi_bounds_to_rows(min_phi - margin, max_phi + margin,
                                             h_virt, height);
      // Slack so a row whose fringe the arc reaches still evaluates the edge:
      // the fringe band of an edge skipped as inactive goes uncovered.
      e.y_lo = static_cast<int16_t>(rows.y_min - EDGE_ROW_SLACK);
  ...
  /**
   * @brief Records the azimuth extent an edge's fringe band is clipped to.
   * @param e Row-crossing record for the edge.
   * @param v1 First arc endpoint (unit).
   * @param v2 Second arc endpoint (unit).
   * @param min_phi Least polar angle the arc reaches.
   * @param max_phi Greatest polar angle the arc reaches.
   * @details Azimuth runs monotonically along a great circle — its projection
   * onto the xz-plane is an origin-centered ellipse — so the endpoints bound
   * the arc's extent. Near a pole the ellipse collapses and the extent may wrap
   * the wrong way, and an extent past a quarter turn cannot be told from its
   * complement; both cases store an inverted range, which disables the clip.
   */
  static void set_arc_azimuth_extent(RowEdge &e, const Vector &v1,
                                     const Vector &v2, float min_phi,
                                     float max_phi) {
    e.th_lo = 1.0f;
    e.th_hi = -1.0f;
    if (min_phi < ARC_POLE_GUARD || max_phi > PI_F - ARC_POLE_GUARD)
      return;
    const float t1 = fast_atan2(v1.z, v1.x);
    float t2 = fast_atan2(v2.z, v2.x);
    if (t2 - t1 > PI_F)
      t2 -= 2.0f * PI_F;
    else if (t1 - t2 > PI_F)
      t2 += 2.0f * PI_F;
    const float lo = t1 < t2 ? t1 : t2;
    const float hi = t1 < t2 ? t2 : t1;
    if (hi - lo > 0.5f * PI_F)
      return;
    e.th_lo = lo;
    e.th_hi = hi;
  ...
  bool get_horizontal_intervals(int y, OutputIt out) const {
    if (row_spans_ok) {
      const RowSpanResult r = emit_row_spans<W, H>(y, out);
      if (r != RowSpanResult::FALLBACK)
        return true;
    }

    // Whole-face azimuth extent: correct for every row, and the fallback
    // whenever the row's crossing set is not a clean pairing.
    if (full_width)
      return false;
  ...
  /** @brief Outcome of the per-row crossing walk. */
  enum class RowSpanResult {
    EMITTED,  /**< Spans were emitted. */
    EMPTY,    /**< No edge reaches the row; nothing to scan. */
    FALLBACK, /**< Crossings unusable; caller uses the whole-face extent. */
  };

  /**
   * @brief Emits row y's coverage spans from the edge/latitude crossings.
   * @tparam W Canvas width in columns.
   * @tparam H Canvas height in rows.
   * @tparam OutputIt Sink type invoked as out(float start, float end).
   * @param y Scan row.
   * @param out Sink accepting (float start, float end) in column units.
   * @return Whether spans were emitted, the row is empty, or the caller must
   *         fall back.
   * @details The painted region is the polygon dilated by the AA reach: its
   * interior, plus a stadium around every edge. Intersecting each edge's great
   * circle with row y's latitude circle and pairing the crossings by winding
   * gives the interior spans, so a row crossing two arms of a star face yields
   * two spans rather than one bridging the notch. The stadia come from the
   * fringe band |n.p| <= reach, clipped to the edge's own azimuth extent — a
   * row grazing an edge lands in that band without ever crossing the edge's
   * circle inside the arc, so the crossings alone do not bound it. Merging both
   * sets yields the row. A row whose crossings are not a clean pairing, or that
   * needs more than MAX_ROW_SPANS spans, falls back to the whole-face extent,
   * as does a fringe wider than MAX_PAD_COLS.
   */
  template <int W, int H, typename OutputIt>
  RowSpanResult emit_row_spans(int y, OutputIt out) const {
    constexpr float REACH_RAD = FRINGE_PX * 2.0f * PI_F / W;
    constexpr float MAX_PAD_RAD = MAX_PAD_COLS * 2.0f * PI_F / W;
    constexpr float TWO_PI_F = 2.0f * PI_F;

    const float sin_phi = TrigLUT<W, H>::sin_phi[y];
    if (sin_phi < ROW_SIN_EPS)
      return RowSpanResult::FALLBACK;
    const float cos_phi = TrigLUT<W, H>::cos_phi[y];
    const float cot_phi = cos_phi / sin_phi;
    // The perpendicular reach as an azimuth offset along this row.
    const float reach_theta = REACH_RAD / sin_phi;
    if (reach_theta > MAX_PAD_RAD)
      return RowSpanResult::FALLBACK;

    float cross_theta[MAX_ROW_CROSS];
    int cross_dir[MAX_ROW_CROSS];
    int num_cross = 0;
    float iv_lo[MAX_ROW_IV], iv_hi[MAX_ROW_IV];
    int num_iv = 0;
    bool any_active = false;

    for (int i = 0; i < count; ++i) {
      const RowEdge &e = row_edges[i];
      if (y < e.y_lo || y > e.y_hi)
        continue;
      any_active = true;

      // n.p runs from +reach to -reach as cos(theta - psi) runs from c + half
      // to c - half, one interval each side of psi.
      const float c = e.k * cot_phi;
      const float half = reach_theta * e.inv_r;
      const float arg_in = c + half, arg_out = c - half;
      if (arg_in >= -1.0f && arg_out <= 1.0f) {
        const float a_in = fast_acos(arg_in > 1.0f ? 1.0f : arg_in);
        const float a_out = fast_acos(arg_out < -1.0f ? -1.0f : arg_out);
        for (int root = 0; root < 2; ++root) {
          float lo = root ? e.psi + a_in : e.psi - a_out;
          float hi = root ? e.psi + a_out : e.psi - a_in;
          if (e.th_lo <= e.th_hi) {
            // The band spans the whole great circle; only the stretch beside
            // the arc itself, widened by the reach, can hold painted pixels.
            const float turn = floorf(
                (e.th_lo + e.th_hi - lo - hi) * (0.25f / PI_F) + 0.5f);
            lo += turn * TWO_PI_F;
            hi += turn * TWO_PI_F;
            if (lo < e.th_lo - reach_theta)
              lo = e.th_lo - reach_theta;
            if (hi > e.th_hi + reach_theta)
              hi = e.th_hi + reach_theta;
            if (hi <= lo)
              continue;
          }
          if (num_iv >= MAX_ROW_IV)
            return RowSpanResult::FALLBACK;
          iv_lo[num_iv] = lo;
          iv_hi[num_iv] = hi;
          ++num_iv;
      ...
      const float a_c = fast_acos(c);
      const Vector &v1 = verts_3d[i];
      const Vector &n = edge_normals[i];
      for (int root = 0; root < 2; ++root) {
        // root 0 is theta = psi - a_c, root 1 is psi + a_c.
        const float ss = root ? -s : s;
        const float ct = e.cos_psi * c + e.sin_psi * ss;
        const float st = e.sin_psi * c - e.cos_psi * ss;
        const float qx = sin_phi * ct, qy = cos_phi, qz = sin_phi * st;

        // Both tests bound the crossing to the v1->v2 arc rather than the whole
        // great circle: within the arc's angle, and on its leading side.
        if (v1.x * qx + v1.y * qy + v1.z * qz < e.arc_cos ||
            (v1.y * qz - v1.z * qy) * n.x + (v1.z * qx - v1.x * qz) * n.y +
                    (v1.x * qy - v1.y * qx) * n.z <
                0.0f)
          continue;

        if (num_cross >= MAX_ROW_CROSS)
          return RowSpanResult::FALLBACK;
        cross_theta[num_cross] = root ? e.psi + a_c : e.psi - a_c;
        // n.p rises through root 0, so that root enters the CCW interior.
        cross_dir[num_cross] = (root == 0) ? 1 : -1;
        ++num_cross;
      }
    }

    // No edge within reach: either the whole latitude ring is interior (only
    // possible when the face wraps a pole) or the row is entirely outside.
    if (!any_active)
      return full_width ? RowSpanResult::FALLBACK : RowSpanResult::EMPTY;
    if (num_cross & 1)
      return RowSpanResult::FALLBACK;

    if (num_cross > 0) {
      if (winding < 0.0f)
        for (int a = 0; a < num_cross; ++a)
          cross_dir[a] = -cross_dir[a];

      // The depth walk pairs along a line, so every crossing must sit on one
      // branch, and centering it on the face keeps the interior off the seam.
      for (int a = 0; a < num_cross; ++a)
        cross_theta[a] +=
            TWO_PI_F *
            floorf((theta_center - cross_theta[a]) * (0.5f / PI_F) + 0.5f);

      for (int a = 1; a < num_cross; ++a) {
        const float tv = cross_theta[a];
        const int dv = cross_dir[a];
        int b = a - 1;
        while (b >= 0 && cross_theta[b] > tv) {
          cross_theta[b + 1] = cross_theta[b];
      ...
      // Off the face's own branch the row is outside, so the walk must open
      // before it closes and must close on itself. A degenerate crossing
      // set — a sub-pixel edge pairing against itself — breaks one of those.
      int run = 0;
      for (int a = 0; a < num_cross; ++a) {
        run += cross_dir[a];
        if (run < 0)
      ...
      for (int a = 0; a < num_cross; ++a) {
        ...
        depth += cross_dir[a];
        if (prev == 0 && depth > 0) {
          open_at = cross_theta[a];
        } else if (prev > 0 && depth == 0) {
          if (num_iv >= MAX_ROW_IV)
            return RowSpanResult::FALLBACK;
          iv_lo[num_iv] = open_at;
          iv_hi[num_iv] = cross_theta[a];
          ++num_iv;
        }
      }
    } else if (full_width) {
    ...
    // One branch for all of them: two intervals covering the same arc a turn
    // apart would sort apart and emit twice instead of merging.
    for (int a = 0; a < num_iv; ++a) {
      if (iv_hi[a] - iv_lo[a] >= TWO_PI_F)
        return RowSpanResult::FALLBACK;
      const float turn = floorf(
          (theta_center - 0.5f * (iv_lo[a] + iv_hi[a])) * (0.5f / PI_F) + 0.5f);
      iv_lo[a] += turn * TWO_PI_F;
      iv_hi[a] += turn * TWO_PI_F;
    }

    for (int a = 1; a < num_iv; ++a) {
      const float lv = iv_lo[a], hv = iv_hi[a];
      int b = a - 1;
      while (b >= 0 && iv_lo[b] > lv) {
        iv_lo[b + 1] = iv_lo[b];
        iv_hi[b + 1] = iv_hi[b];
        --b;
      }
    ...
    float span_start[MAX_ROW_SPANS], span_end[MAX_ROW_SPANS];
    int num_spans = 0;
    float open_lo = iv_lo[0], open_hi = iv_hi[0];
    for (int a = 1; a <= num_iv; ++a) {
      if (a < num_iv && iv_lo[a] <= open_hi) {
        if (iv_hi[a] > open_hi)
          open_hi = iv_hi[a];
        continue;
      }
      if (num_spans >= MAX_ROW_SPANS)
        return RowSpanResult::FALLBACK;
      span_start[num_spans] = open_lo;
      span_end[num_spans] = open_hi;
      ++num_spans;
      if (a < num_iv) {
        open_lo = iv_lo[a];
        open_hi = iv_hi[a];
    ...
    const float scale = W / TWO_PI_F;
    for (int a = 0; a < num_spans; ++a)
      out(floorf(span_start[a] * scale), floorf(span_end[a] * scale) + 1.0f);
    return RowSpanResult::EMITTED;
  }
