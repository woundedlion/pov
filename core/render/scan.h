...
/**
 * @brief Rasterizes the band swept by a great-circle ring whose normal rotates
 *        from n0 to n1 along the geodesic between them.
 */
struct RingSweep {
  /**
   * @brief One swept-band span: endpoint normals plus shading parameters.
   */
  struct Span {
    ...
    float positions; /**< Sub-positions merged into this span (dwell weight). */
    bool draw_far_end; /**< Draw the theta = Theta endpoint fringe (final
                          span); interior boundaries defer it to the next
                          span so shared rings are not double-blended. */
  };
   ...
   *         with t the trail parameter of the covering ring position.
   * @param pipeline Plotting pipeline receiving the final colors.
   * @param canvas Destination canvas.
   * @param span Swept-band span to rasterize.
   * @param shader Per-pixel shader (see SweepShaderT).
   * @details Per pixel, d(theta) = dot(p,n0)cos + dot(p,b)sin with b = a x n0
   * (a the geodesic axis), so n(Theta) = n1 exactly and a crossing inside the
   * sweep is just sign(d0) != sign(d1) — the covered-pixel test costs two
   * dots. Crossing pixels lie on an interior swept ring (dist 0); the others
   * see the nearest endpoint ring. Stacked-trail brightness is reproduced by
   * compositing (1 - (1-a)^n_eff) where n_eff is the sub-positions whose ring
   * passes within the stroke band at this pixel. Requires sweep angle < pi/2
   * (one zero of d per window); the caller's span partition enforces it.
   */
  ...
  static void draw(PipelineT &pipeline, Canvas &canvas, const Span &span,
    ...
    // Covering ring: every swept normal lies within theta_total/2 of the mid
    // normal, so the mid great circle inflated by that spread plus the stroke
    // contains the whole band. The 1/0.95 counters Ring's 0.95*thickness
    // bounds shave — AA dust for a real stroke, but core endpoint-ring rows
    // for a cover whose width is mostly sweep spread.
    ...
    // Sub-ring density per radian of sweep; n_eff = density * band width.
    const float density =
        degenerate ? span.positions : (span.positions - 1.0f) * inv_theta;
    ...
    auto shade = [&](int x, int y, const Vector &p, float dist, float t_par,
                     float n_eff) {
      const float aa = quintic_kernel(1.0f - dist * inv_th);
      if (aa <= 0.001f)
        return;
      frag.color = Color4(0, 0, 0, 0);
      frag.pos = p;
      ...
      frag.age = 0;
      shader(t_par, p, frag);
      float a1 = frag.color.alpha * aa;
      if (a1 <= 0.001f)
        return;
      // 1-(1-a)^n for fractional n in [1,4] — u^n interpolated linearly
      // between consecutive integer powers so the dwell weight stays smooth
      // (an integer-branched power contours into visible brightness tiles).
      float u = 1.0f - a1;
      float u_lo = u, u_hi = u * u, base = 1.0f;
      if (n_eff >= 3.0f) {
        base = 3.0f;
        u_lo = u_hi * u;
        u_hi = u_lo * u;
      ...
      float frac = n_eff - base;
      float out_a = 1.0f - (u_lo + (u_hi - u_lo) * frac);
      pipeline.plot(canvas, x, y, frag.color.color, frag.age, out_a);
    };
    ...
    auto pixel_run = [&](int x1, int x2, int y, float sp, float cp) {
      for (int x = x1; x < x2; ++x) {
        ...
        if (!degenerate && ((d0 > 0.0f) != (d1 > 0.0f))) {
          // Interior crossing: p lies on a swept ring. theta* solves
          // d0 cos + B sin = 0.
          const float B = dot(p, b);
          // tan(theta*) = -d0/B, period pi; lift the atan2 branch into [0, pi)
          // where the single in-window zero lives.
          float th_star = fast_atan2(-d0, B);
          if (th_star < 0.0f)
            th_star += PI_F;
          float t_hit = hs::clamp(th_star * inv_theta, 0.0f, 1.0f);
          const float m = std::sqrt(d0 * d0 + B * B);
          // th (not 2*th): stacked sub-rings composite through their quintic
          // AA, whose band mean is 1/2 — an unweighted in-band count would
          // render the smear visibly brighter than the stacked original.
          const float n_eff = hs::clamp(density * th / (m + 1e-6f), 1.0f, 4.0f);
          shade(x, y, p, 0.0f, span.t0 + (span.t1 - span.t0) * t_hit, n_eff);
          continue;
        }
        const float a0 = std::abs(d0);
        const float a1 = std::abs(d1);
        float dist = a0 <= a1 ? a0 : a1;
        if (dist >= th)
          continue;
        const bool far_end = a1 < a0;
        if (degenerate) {
          shade(x, y, p, a0, span.t1, hs::clamp(span.positions, 1.0f, 4.0f));
          continue;
        }
        if (far_end && !span.draw_far_end)
          continue;
        // Endpoint dwell from the sweep speed |d'| at that end; equals the
        // crossing weight as dist -> 0 (M -> |B| there), so the two shading
        // regions meet without a brightness step.
        const float B = dot(p, b);
        const float deriv =
            far_end ? std::abs(B * cos_full -
                               d0 * std::sqrt(std::max(
                                        0.0f, 1.0f - cos_full * cos_full)))
                    : std::abs(B);
        const float n_eff = hs::clamp(
            density * 0.5f * (th - dist) / (deriv + 1e-6f), 1.0f, 4.0f);
        shade(x, y, p, dist, far_end ? span.t1 : span.t0, n_eff);
      }
    };
    auto run_clipped = [&](int x1, int x2, int y, float sp, float cp) {
      if (!xc.active) {
