...
#include "platform/diagnostics.h" // HS_OS_CYCLES, hs::log
#include <cstdint>
#include <cstring>
...
struct PlotCounts {
  uint32_t rings = 0;              /**< Ring paths submitted. */
  ...
  uint32_t one_dot = 0;            /**< Single-sample paths. */
  uint32_t cull_tests = 0;         /**< Path cull tests. */
  uint32_t culled = 0;             /**< Paths rejected by culling. */
  uint32_t sim_samples = 0;        /**< Samples generated by simulation. */
  uint32_t replay_samples = 0;     /**< Samples emitted during replay. */
  uint32_t planar_unprojects = 0;  /**< Planar-to-world conversions. */
  uint32_t planar_arc_samples = 0; /**< Samples on planar arcs. */
  uint32_t normalizations = 0;     /**< Vector normalizations. */
  uint32_t shader_calls = 0;       /**< Fragment-shader invocations. */
  uint32_t plotted_samples = 0;    /**< Samples reaching the framebuffer. */
  uint32_t steps_peak = 0;         /**< Peak cached path steps. */
  uint32_t backstops = 0;          /**< Capacity backstop trips. */
...
struct MindSplatterCounts {
  ...
  uint32_t prologue_row_rejects = 0;    /**< Row-bound prologue rejects. */
  uint32_t prologue_column_rejects = 0; /**< Column-bound prologue rejects. */
  uint32_t edge_rejects = 0;            /**< Edges rejected before sampling. */
  uint32_t visible_trails = 0;          /**< Trails reaching rasterization. */
  uint32_t one_dot_edges = 0;           /**< Trails reduced to one sample. */
  uint32_t long_edges = 0;              /**< Trails using adaptive sampling. */
  uint32_t adaptive_samples = 0;        /**< Adaptive samples emitted. */
  uint32_t exact_gate_fallbacks = 0;    /**< Exact visibility fallbacks. */
  uint32_t fragment_shader_calls = 0;   /**< Fragment-shader invocations. */
  uint32_t aa_tap_masks[5]{};           /**< AA masks by populated tap count. */
  uint32_t interior_splats = 0;         /**< Splats wholly inside the clip. */
  uint32_t clip_boundary_splats = 0; /**< Splats crossing the clip boundary. */
  uint32_t palette_endpoints = 0;    /**< Endpoint palette samples. */
  uint32_t palette_interpolated = 0; /**< Interpolated palette samples. */
  uint32_t hole_early_outs = 0;      /**< Hole-mask early exits. */
...
struct MindSplatterStalls {
  ...
  DwtStallBucket adaptive_sim;        /**< Adaptive simulation. */
  DwtStallBucket normalized_replay;   /**< Normalized trail replay. */
  DwtStallBucket shade_palette;       /**< Palette lookup and shading. */
  DwtStallBucket projection;          /**< World-to-screen projection. */
  DwtStallBucket aa_weights;          /**< Antialias weight evaluation. */
  DwtStallBucket framebuffer_blend;   /**< Framebuffer blending. */
  DwtStallBucket signed_axis_physics; /**< Signed-axis particle update. */
