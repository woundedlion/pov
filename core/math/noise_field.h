...
enum class NoiseChannelLayout : uint8_t { SCALAR_V1, DIRECT_V1, CURL_V1 };
...
inline float sample_noise_octaves(const FastNoiseLite &noise, NoiseBasis basis,
                                  const Vector &q) {
  const float first = noise.GetNoise(q.x, q.y, q.z);
  if (basis == NoiseBasis::SIMPLEX)
    return first;
  const float second = noise.GetNoise(2.0f * q.x, 2.0f * q.y, 2.0f * q.z);
  const float third = noise.GetNoise(4.0f * q.x, 4.0f * q.y, 4.0f * q.z);
...
inline float sample_noise_vector_channel(const FastNoiseLite &noise,
...
inline Vector sample_direct_tangent(const FastNoiseLite &noise,
                                    NoiseBasis basis, const Vector &q,
                                    const Vector &v, float direction) {
  Vector u(sample_noise_vector_channel(noise, basis, q, 0),
