...
`gen/pcb.py --unplaced --force` regenerates the board and updates the project
from `gen/constraints.py`. There is no unplaced
...
- After downloading candidates, run `python gen/analyze_candidates.py <paths>`.
  ...
  **re-widen the four sync lands only for a stock-land rev 1.1 board** (below),
  then run `python gen/fab.py`;
  fabrication output repeats the same via gate.

**Stock-land rev 1.1 re-routes only: re-widen the four sync lands.**
The committed rev 1.1 `unplaced/phantasm_unplaced.kicad_pcb` stages `R1`, `R2`,
`R_PD` and `R_S` on stock library lands. Boards regenerated by `board.py` for
rev 1.2 embed HandSolder lands: do not widen them, because added sub-body copper
violates the rev 1.2 land requirements. On promotion of a regenerated board,
re-baseline `SHIPPED_CHIP_LANDS` to its HandSolder ids and geometry.
For a stock-land board, edit the promoted `phantasm.kicad_pcb` in place —
