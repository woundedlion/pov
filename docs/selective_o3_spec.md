...
| ShaderBall-local R4 reclaim: outer pixel lambda + `sample_pattern` `HS_O3_FN` | **+304** (full Phantasm 193,480 → 193,784) | 2,824 (was 3,128) | full-cycle shipping peak **55.97 → 53.99 ms**, 0/6,368 spills; clean preset-6 `sb_shader_draw` 52.73 ms in the baseline report → 50.87 ms measured combined. Against the fresh 50.20 ms global-O3 twin, the original gap is 5.77 ms and the residual gap is 3.79 ms: **34.3% reclaimed**. The macro is empty in the global-O3 build, so its codegen is unaffected | ✅ `17fbf726` 2026-08-08 |
| Typed ShaderBall stage helpers: `shade`, `shade_projected`, `project_branch`, `planar_warp_lookup`, `warp_stage_lookup`, and `colorize` `HS_O3_FN` | **+0 in full Phantasm** (effect excluded); +4,192 B in the single-effect profile image | 5,000 B in full Phantasm (unchanged) | fixed mixed preset 6 peak **56.68 → 53.99 ms** and `sb_shader_draw` 51.54 → 48.85 ms; fixed Airocean preset peak **43.57 → 40.10 ms**; all captures 0 spills. Removing the two smaller `planar_warp_lookup`/`colorize` attributes saved 528 B but gave back ~0.65 ms, so the measured six-helper boundary remains | ✅ 2026-08-09 |
