...
| 6 | **`HS_OS_CYCLES()`** (`core/engine/platform.h:171-175` = `ARM_DWT_CYCCNT` under `CORE_TEENSY` / `:383` = `0` host) | `ARM_DWT_CYCCNT` (device) / `0` (host). | **No** — a profiling timestamp; never affects render output. | ⚪ **N/A.** | Non-behavioral. |
| 7 | **`StaticCircularBuffer` host-only narrowing** (`core/engine/static_circular_buffer.h:479-481`) | `head`/`tail`/`count` are `uint32_t` rather than `size_t`, so pooled structs keep one layout on both targets. On the device `size_t` *is* `uint32_t`, making the narrowing host-only; identical codegen and behavior on hardware. | **No.** | ⚪ **N/A.** | Non-behavioral. |
