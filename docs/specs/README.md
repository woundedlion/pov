...
|---|---|
| [pullback_pipeline_spec.md](pullback_pipeline_spec.md) | LANDED. The composition core, standard carriers, provider concepts, stage combinators, and operator catalog that ship under `core/render/pullback/`, with `core/render/pullback.h` as the public umbrella. Field-level source of truth for the carrier records (Sphere, Plane, Field, Color) the shader family composes over. §17 is a design record with no implementation. |
