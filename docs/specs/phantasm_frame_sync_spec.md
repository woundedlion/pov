...
the current entry's revolution budget elapses, emits the **EPOCH** symbol at
boundary B.
...
1. **Redundancy:** the EPOCH symbol is repeated on the next R ZERO-boundaries
   ...
   `rev_in_effect` reaches the entry's budget
   (`Config::revolutions_for_effect`), so a board hearing copy j infers j from its
...
5. **Lifecycle & effect handoff.** The flywheel and sync-wire ISRs attach
   **once per show and persist across epochs** — the flywheel *is* the
   ...
   (a relaxed `std::atomic<uint32_t>`, `generation << 8 | index`); on a generation change
