...
1. ❌ **[effects] `effects/PetalFlow.h:47`** — Rejected: no seam is visible in PetalFlow, and the AntiAlias tap already has sufficient margin at the band boundary, so `full_frame` is not needed here. Constructor passes `{.strobe = true}` but omits `.full_frame = decltype(filters)::any_crosses_segments`, even though the pipeline contains `Filter::Screen::AntiAlias<W,H>` (line 51). It is the only AntiAlias-carrying effect that fails to set the flag; siblings RingShower/ShapeShifter/SplineFlow/Thrusters all set it. Under segmented (Phantasm) rendering the anti-aliaser reads across a band boundary that isn't coherent, producing visible seams along the petals at every band edge, every frame. *Fix:* change the base init to `Effect(W, H, {.strobe = true, .full_frame = decltype(filters)::any_crosses_segments})`.
