...
- **`Liquid2D`'s pattern time comes from an `Animation::Driver` that wraps at 1.0** while its sibling `Flyby` uses an unbounded accumulator; if the Driver wraps, the pattern hitches once per cycle. Worth confirming against `animation.h` and unifying.

Neither of the first two crashes today; all three are real.
...
No memory-safety defects (use-after-free, OOB, alignment, arena overflow) were found in the engine core; the arena bounds math is provably wrap-safe and the comment proves it. Numerical edges are guarded with uncommon consistency — every stereographic projection, normalize, and division on a hot path has a documented epsilon or `normalized_or` fallback, and the NaN/Inf fault cluster is death-tested to *trap* rather than propagate. Held back from the A band by three real latent edges (feedback-flush clip, segmented frame-sync buffer-flip, Liquid2D time-wrap) and a scatter of low-severity issues: `BakedPalette` truncation bias, unbounded `Color4` alpha, the un-narrowed `hankin.h:196` face count, and the unverified `PROGMEM`/RAM placement of the 301 KB reaction-graph table.
...
13. Extract a `StereoShaderBase` CRTP for Liquid2D + Flyby (largest single dedup); factor the live-param-sync and orientation-trail-render idioms; unify the two `Scan::Shader` SSAA loops; call `quintic_kernel()` from DistortedRing.
14. Reconcile README drift: the WASM divide-by-65535 sample, stale effect parameter lists (IslamicStars/Metaballs/Voronoi/MobiusGrid), and the §7.10 segmented row-range wording.
15. Add `test_util.h`, `test_presets.h`, and direct `styles.h` coverage; extend the death harness to the deferred memory/spatial/circular-buffer misuse preconditions.
16. Delete dead code: `ReactionDiffusionBase::H_VIRT`, the unused `<functional>` includes, the stale using-decls, and the unreferenced `hs::H_OFFSET`.
