...
2. **Hardware DMA end-frame + overrun hazards** (`dma_led.h:55-57,381-391`). `END_FRAME_BYTES = (N/2)+1` does not match the documented `ceil((N+1)/2)` for all N (latent on odd geometries), and on a DMA overrun `submitFrame()` *busy-waits inside the column ISR* (`waitComplete`) rather than dropping the frame — a real-time hazard that can cascade missed columns on Phantasm. **Fix:** make `END_FRAME_BYTES` match the spec with a `static_assert` on `BUFFER_SIZE`, and on overrun drop the frame (return without flipping) instead of spinning in interrupt context.
