...
19. ✅ Reconcile README drift: the WASM divide-by-65535 sample, stale effect parameter lists (IslamicStars/Metaballs/Voronoi/MobiusGrid), and the §7.10 segmented row-range wording. *Fixed: both "divide by 65535 in JS" samples (§5 WASM-path block and §10.2) now describe the real path — the zero-copy `Uint16Array` is bound as the instanced dot-mesh's `normalized` `instanceColor` attribute and the GPU scales 0–65535→0–1, with no JS divide/`Float32`/`DataTexture` (matches `driver.js`). §10.7 now calls each worker's region a quadrant (one arm's half-width × a Y-band) instead of a full-width "Y-band", matching `computeSegmentRange()`. The four effect parameter lists were validated against the actual `registerParam` calls and already match exactly — left unchanged.*
20. ✅ Add `test_util.h`, `test_presets.h`, and direct `styles.h` coverage. *Fixed: three new self-contained test headers wired into `run_tests.cpp`. `test_util.h` covers both `wrap` overloads, `wrap_t`, `fast_wrap` (across its full legal window), `shortest_distance`/`fwd_distance`, and `apply_if_changed`. `test_presets.h` covers `Presets<>` get/next/prev wraparound, `prev_get`/`apply`, single-entry in-place wrap, and CTAD size deduction. `test_styles.h` covers `Feedback::Style` named presets, `lerp` scalar interpolation + function-pointer/discrete snapping + bound-pointer preservation, the warp/fade transforms, and `sync_noise`. All green (346 new checks).*
