...
24. ✅ `effects/GSReactionDiffusion.h:135` — `STEPS_PER_FRAME = 16` (vs BZ's 2) is the single largest per-frame cost and reads as arbitrary. → Add a one-line justification for the 16× substep count. *(Documented the rationale: GS morphogenesis is slow per step — the feed/kill reaction (feed ~0.04, kill ~0.06) and small diffusion rates move the field only fractionally each substep — so 16 substeps/frame are needed for the pattern to grow/drift visibly, where BZ's faster oscillator reads in 2. Each substep stays inside the explicit-Euler stability bound already documented at the "Speed" (dt) param, so the count is evolution throughput, not a stability risk. Comment-only.)*
25. ✅ `effects/FlowField.h:158` — the 600-particle × 3-noise-call per-frame force loop is the field group's second-heaviest cost and, unlike `Raymarch`, carries no device budget note. → Add one or confirm it fits the per-column budget. *(Added a budget note at `k_num_particles`: the force loop is a fixed ~k_num_particles*3 (~1800) OpenSimplex2 evaluations/frame and — unlike Raymarch's per-pixel marching — is independent of W*H, so it carries no resolution cost cliff and fits the device frame budget at any supported resolution, with the knob to lower it called out. Confirmed the cost is per-frame (the emitter runs once per `particle_system.step`), not per-column/per-pixel. Comment-only.)*
26. ❌ `effects/Dynamo.h:189-252,305` — `color()` linearly walks every active palette boundary and is invoked per replayed trail point, giving `O(points × boundaries)` `angle_between`+LUT scans on the hot path. → Hoist the per-flush band lookup or precompute; profile on device. *(Rejected — the premise overstates the cost and the proposed fix is infeasible. `angle_between` is computed **once per point** (Dynamo.h:198), outside the boundary loop — so it is O(points) acos, not O(points × boundaries). The per-boundary work is only cheap float compares with an early `return`; the boundary count is `palette_boundaries` ≤ `MAX_PALETTES - 1` = 15 and in steady state 0, in which case the loop body never executes and `color()` is O(1) (returns `baked_palettes_[0].get(t)` directly). Boundaries are non-zero only transiently while wipes are in flight (typically 1–3). Crucially, the band cannot be hoisted out of the per-point loop or precomputed across points: it is selected by `a = angle_between(v, palette_normal)`, which depends on the per-point direction `v`, and the returned `.get(t)` uses the per-point age `t` — both vary per replayed trail point. The only per-point cost worth anything is the single `acos` in `angle_between`, which is inherent to angle-based coloring and removing it would change the visual. No free win, so no change.)*
27. ✅ `effects/MindSplatter.h:107-115` — the pool-footprint comment's hand arithmetic ("~19 KB headroom") is thin and rounds KiB/KB loosely while that margin must also cover scratch, palette, and emitters. → Verify actual `sizeof` and add a `static_assert` on the pool against the carved arena. *(Verified the layout exactly: `StaticCircularBuffer<Vector,23>` = 23*12 + 3*uint32 = 288 B, so `sizeof(Particle<23>)` = 12+12+2+2+288 = 316 B and the 1024-pool = 316 KiB; after the 11 KiB scratch carve the 335 KiB device arena leaves ~8 KiB persistent for the baked palette (~3 KiB LUT) + attractor/emitter vectors (<1 KiB). Added a compile-time `static_assert` in `init()` checking `sizeof(Particle<kTrailLen>)*NUM_PARTICLES + 6 KiB aux reserve <= 335 KiB - SCRATCH_BYTES`, deriving the pool size from `sizeof` (re-checks on any layout change) and named the trail length `kTrailLen` so the typedef and the assert track together. The guard uses the **device** arena literal because `GLOBAL_ARENA_SIZE` is inflated to 8 MiB on the host test build, mirroring how the GS/BZ effects pin their persistent budget. Rewrote the loose comment with the exact figures.)*
...
33. ✅ `daydream.js:452-469` — `applyResolution()` can double-apply the effect: the forced `appState.set('effect', …)` fires `applyEffect()` synchronously *before* `updateResolution()` rebuilds the dot mesh/sidebar, so the effect GUI is constructed mid-resize. → Reorder so the effect switch happens after `updateResolution()`. *(daydream `5220c01`: moved the effect-switch block below `updateResolution()`/`sidebar.setEffects()`, so the synchronous `applyEffect()` fired by the set() builds against the rebuilt mesh/sidebar.)*
34. ✅ `daydream.js:457` — the resolution-preset field named `size` is actually the dot *radius*, inviting misreading as pixel count. → Rename to `dotSize`. *(daydream `4589a8f`: renamed the preset field `size`→`dotSize` at both definitions and the `updateResolution(h,w,dotSize)` call site, matching the param name driver.js already used.)*
35. ✅ `tools/lissajous_math.js:40-46` — `lissajous()` omits the `.normalized()` the engine applies (`geometry.h:760`), so the JS preview diverges slightly from the device off the `sin(m2·t)≈0` points. → Apply the same normalization (a parity hazard for an export tool). *(daydream `3d6d6c4`: added `.normalize()` to the returned `THREE.Vector3`, mirroring the engine's `v.normalized()`; existing lissajous parity/unit-sphere tests still pass.)*
36. ✅ `tools/lissajous_math.js:54-83` — `findBestRationalRatio` only scans positive `M/N`, so a negative target ratio snaps to the wrong (sign-flipped) figure. → Take `abs` and reapply `sign`, or document non-negative inputs. *(daydream `be19203`: split the sign off, search on `|value|`, carry the sign back on the numerator (N stays positive); added negative-target tests for `findBestRationalRatio` and `snapToRationalRatio`.)*
37. ✅ `daydream/tools/color.js` — `srgbToOklch`/`oklchToLinearRgb` are asserted only for finiteness/range, not against golden reference values, so a wrong-but-finite matrix or hue error passes. → Pin a golden OKLCH triple per function. *(daydream `7afde87`: added golden tests — sRGB red → canonical Ottosson OKLCH (L 0.62796, C 0.25768, h 0.51023 rad), and an in-gamut OKLCH inverting to its known linear RGB. Verified the matrices match the canonical values.)*
38. ✅ `daydream/tests/color_parity_wasm.test.js:89` — the palette parity tolerance is ±2 16-bit LUT steps, looser than the "within one step" the docstring claims; a 2-step systematic bias passes as parity. → Tighten to ±1 (or justify) and add a fixed golden value to catch a uniform offset. *(daydream `6596684`: measured max divergence over the grid is exactly 1 step, so tightened to `<=1` (matching the docstring); added an absolute golden-value test pinning `procedural_palette_linear` at t=0.5 and t=0.25, which the wasm-vs-js parity check cannot catch for a uniform shared offset.)*
39. ✅ `platformio.ini:69` vs `CMakeLists.txt:71` — the device firmware builds without `-ffast-math` while WASM builds with it, so simulator float results are not bit-identical to the device (presumably intended given the integer-wrap-guard parity contract). → Confirm acceptable and document where a reader of either build file would see it. *(Resolved by **matching** the device to WASM: added `-ffast-math -fno-finite-math-only` to the shared `[env].build_flags` (so both holosphere and phantasm carry it), with a comment mirroring the WASM `-fno-finite-math-only` NaN-guard rationale and noting float results are still NOT bit-identical across ARM/WASM (the parity contract is the integer wrap guards). Added a reciprocal cross-reference in `CMakeLists.txt`. Verified via `pio run`: holosphere builds and PASSES the size gate (RAM1 free 97,984 B); the flag is byte-neutral on phantasm RAM1 (code −144 B, identical `free`), whose ~48 KB RAM1 overflow is a pre-existing condition on master being addressed by separate ongoing size-reduction work, not introduced here.)*
...
44. ❌ `tests/test_solids.h` — Catalan/Islamic solids are checked finite/non-empty but not asserted off-sphere, so a bug wrongly snapping `hankin`/`expand` vertices back to the unit sphere passes. → Assert some vertices are measurably off-sphere. *(Rejected — premise is factually wrong. All 23 registry Islamic solids build vertices ON the unit sphere: measured max |‖v‖−1| = 1.2e-7 across the family. `finalize_solid` does no normalization, so this is the generators' genuine output — these hankin/expand star patterns live on the sphere surface by construction. An "off-sphere" assertion is therefore false and was empirically observed to fail. Catalan solids are already pinned on-sphere via `check_all_unit_vertices`. The file's existing "magnitude is NOT asserted" comment correctly reserves against a future genuinely-open Islamic entry, so flipping it to an on-sphere assertion was declined to avoid a false alarm on such an addition.)*

### Low

45. ✅ `core/static_circular_buffer.h:62` — the initializer-list overflow path calls `hs::log` unconditionally from a constructor that may run before `Serial.begin()` on device. → Gate behind `hs::debug` or document post-setup-only. *(Went further than gating: replaced the `std::initializer_list` constructor with a variadic-template one where the element count is a compile-time `sizeof...(Args)`, so an over-capacity literal is now rejected by `static_assert` instead of logging+truncating — the `hs::log` call is gone entirely, removing the pre-`Serial` hazard. A `std::initializer_list` ctor cannot static_assert (its `size()` is a runtime property of a function parameter, not a constant expression); the parameter pack can. The `is_constructible<T, Args&&>` enable_if guard keeps the variadic ctor from shadowing the copy/move/default constructors, and elements are built `T{...}` to preserve narrowing diagnostics. The former runtime "keep last N on init overflow" path is replaced by `test_initializer_list_at_capacity` since over-capacity is now a compile error. All 34 native tests pass.)*
46. ✅ `core/static_circular_buffer.h:174-188` — internal `pop_*` paths decrement `count` with no `HS_CHECK`, relying on caller gating. → Add a symmetric guard or document the internal caller-guaranteed contract. *(Documented the caller-guaranteed contract on both `pop_back_internal`/`pop_front_internal`: spelled out the gating call sites (`pop_back`/`pop`/`clear` on emptiness, the `push_*`/`emplace_*` eviction paths on fullness) and that an empty-buffer call would wrap `count` (uint32_t) past 0 and corrupt `size()`, so the gating is load-bearing. No guard added — the hot-path doctrine keeps these branchless.)*
47. ✅ `core/concepts.h:296` — `ScalarFn = Fn<float(float),32>` is the lone callable-capacity choice with no rationale comment while every sibling is justified. → Add a one-line note on the 32-byte budget. *(Added the rationale: ScalarFn holds the wave/shift-builder closures — `sin_wave` & friends capture from/to/freq/phase (4 floats) and `DistortedRing` `shift_fn` / `Animation::Mutation` closures carry a few params more — all past the 16-byte sprite-capture budget, so 32 covers them with headroom.)*
48. ✅ `core/inplace_function.h:139` — the static_assert message says "trivial POD/pointer closures" but the class accepts any nothrow-copyable callable. → Soften the wording. *(Reworded: "Store only nothrow-copyable callables — any qualifying type is accepted (in practice lambdas capturing PODs/pointers, which are trivially so)." — states the real constraint instead of implying the set is limited to trivial POD/pointer closures.)*
49. ✅ `core/util.h:54` — `wrap(long,long)`/mixed-integer pairs route through `float fmod` and silently lose precision above 2²⁴. → Document that only the `(int,int)` overload is exact. *(Added an exactness note to the template `wrap`: the body wraps via `std::fmod` (floating-point even for an integral common type — integer args promote to `double`), so it is exact only within the working mantissa (~2²⁴ when the common type is `float`, ~2⁵³ on the `double` path); large pairs like `wrap(long,long)` lose precision past 2⁵³, and only the dedicated `wrap(int,int)` integer-`%` overload is exact across its full range.)*
50. ✅ `core/easing.h:18-21` — cubic easings pass `t` unclamped and can return out-of-`[0,1]` for out-of-range input the sibling comments invite. → Confirm downstream clamps palette indices or note the easings are unclamped by design. *(Added a file-level convention note: every easing takes `t` in `[0,1]` and is unclamped by design (matching the easings.net reference), neither clamping input nor bounding output — so out-of-range `t` extrapolates and the cubic/back/elastic variants can exceed `[0,1]`; callers feeding an unbounded drive or indexing a fixed table must clamp `t`/the output themselves.)*
51. ✅ `core/waves.h:25` — `sin_wave` recomputes the loop-invariant `2π·phase` term per call. → Precompute in the capture if it ever moves to a hot loop. *(Hoisted the exact `2*PI_F*phase` subterm into a `phase_term` capture, computed once at build. Bit-identical: only that subterm is hoisted (capture stays 4 floats, no ScalarFn-budget growth); the `freq·t·2π` product is left inline since reassociating it could shift the last bit, and the wave must stay sim/device bit-identical. `test_sin_wave_bounds_and_phase` still passes.)*
52. ✅ `core/3dmath.h:23` — single-letter global `G` for the inverse golden ratio is easy to confuse. → Rename to `INV_PHI`. *(Renamed the constant `G` → `INV_PHI` and updated all four use sites: `color.h` hue-seed stride, `geometry.h` golden-angle theta, `effects/MindSplatter.h` hue advance, and the `test_3dmath.h` constant test. No bare-`G` constant references remain (the unrelated `R/G/B` comments, `pov_sync` gate doc, and `test_sdf` local grid `G` are untouched).)*
53. ❌ `core/3dmath.h:1176` — Vector `slerp` antipodal fallback returns `v1` regardless of `t`. → Consider `t < 0.5 ? v1 : v2` so the degenerate path honors direction. *(Rejected — premise is stale. The antipodal branch is `return normalized_or((v1 + (v2 - v1) * t), v1);` (3dmath.h:1191): it interpolates v1→v2 by `t` and normalizes, so it already honors `t` across the whole range. `normalized_or` only substitutes `v1` when the lerp magnitude underflows to ~0 — i.e. the exact antipode at `t≈0.5`, where the great-circle direction is genuinely undefined. The suggested `t < 0.5 ? v1 : v2` would be a strict regression: it snaps to an endpoint for every `t` instead of interpolating, and is no better than the current code at the single undefined midpoint (it would return `v2` there vs `v1` — both arbitrary). No change.)*
54. ✅ `core/geometry.h:304-314` — the float `y_to_phi` overload does not clamp `y` to row bounds (the int overload traps). → Document it is unchecked by design or add a debug assert for symmetry. *(Documented as unchecked by design: extended the `@details` to note that, unlike the integer `y_to_phi<H>(int)` overload which HS_CHECKs y in `[0, H_VIRT)`, this fractional overload neither clamps nor traps an out-of-range y — only the in-bounds near-integer case takes the LUT, and any other y falls through to the analytic formula which extrapolates linearly; sub-pixel rows are fed here intentionally and an out-of-range row is the caller's responsibility. No assert added — the float path takes deliberately unbounded sub-pixel input.)*
55. ✅ `core/geometry.h:802-810` — `get_antipode` negates two basis axes (preserving handedness) but the result no longer satisfies `make_basis`'s `w = cross(v,u)` relationship. → Verify no consumer recomputes `w` from `u,v`. *(Premise corrected: the relationship **is** preserved — `cross(-v, -u) == cross(v, u)`, so the kept `w` stays consistent with the negated `u/v`. Verified the consumers too: every `get_antipode` caller in `plot.h`/`scan.h` uses the returned basis directly, and the two `w = cross(v,u)` recomputations (`plot.h:1147,1538`) build their **own** planar basis from `-basis.v` independently of `get_antipode`, so none recompute `w` from its result — and even one that did would get the same vector back. Added a comment to `get_antipode` documenting the surviving invariant so a future editor doesn't "fix" it.)*
56. ✅ `core/spatial.h:25-119` — `AABB` is documented dead code ("no production consumer yet"). → Consider relocating to a `spatial_experimental.h` so the core header holds only wired-up code. *(Removed the dead `AABB` struct outright (per request) rather than relocating it: it had no production consumer — only `spatial.h` defined it and `test_spatial.h` exercised it. Deleted the struct from `core/spatial.h` and its nine `test_aabb_*` cases + their runner calls + the AABB mentions in the file header from `tests/test_spatial.h`; `KDTree`/`MeshState` (the wired-up types) and all includes are untouched. The only surviving "AABB" reference is the unrelated generic doc-comment in `constants.h`. Native suite green.)*
57. ✅ `core/spatial.h:240,261` — inconsistent `size_t` casting on `k` two lines apart. → Pick one. *(Made both comparisons cast `k` to `size_t`: `get_worst_dist`'s `best.size() < k` now matches `offer_candidate`'s `best.size() < static_cast<size_t>(k)`, so the size-vs-`k` comparison reads identically in both lambdas and neither leans on an implicit signed→unsigned promotion.)*
58. ✅ `core/reaction_graph.h:113` — `lookup()` divides by the dominant-axis magnitude with no zero-guard, safe only because a unit `p` guarantees `≥1/√3`. → Note the unit-length precondition is load-bearing for the divide. *(Rewrote the `@param p` doc to state the unit-length requirement is load-bearing: the dominant-axis magnitude is the unguarded divisor and only a unit `p` keeps that axis `≥ 1/√3` (~0.577) so the reciprocal can't blow up; a zero/denormal `p` would divide by ~0.)*
59. ✅ `core/color.h:1453` — `g_hue_seed` is a mutable static shared across all `GenerativePalette` instances (documented single-threaded; safe today because each WASM worker has its own linear memory). → Consider threading the seed through the constructor so the global is opt-in. *(The constructor already threads a seed: passing `manual_seed >= 0` makes the global opt-out — that path neither reads nor advances `g_hue_seed`, yielding an instance-local deterministic base hue. Documented this on both the `manual_seed` param and the `g_hue_seed` declaration rather than change the auto-seed default, which deliberately drives hue variety across palettes.)*
60. ✅ `core/color.h:1106` — calling the fixed coprime additive step (157 mod 256) a "low-discrepancy stride" overstates it; it is an R1 additive recurrence. → Reword. *(Reworded the comment to name it an R1 additive recurrence (Weyl sequence) mod 256 with integer increment 157, and to state plainly that "low-discrepancy" overstates it — the true low-discrepancy property needs the exact irrational increment, which truncating to 157 discards, so equidistribution is only approximate.)*
61. ✅ `core/styles.h:166-173` — `sync_noise() const` mutates through bound state while sibling `sync_hue()` is non-const. → Add a note that constness reflects writes-through-bound-state vs own-members. *(Added a doc note: `sync_noise()` is const because it leaves the Style's own members untouched and writes only through the bound `noise` pointer; `sync_hue()` is non-const because it mutates the Style's own `hue_ca`/`hue_sa` cache. The constness reflects own-members-vs-through-pointer, not a sync distinction.)*
62. ❌ `core/memory.h:237` — `TriangularBitset::index()` uses a debug-only `assert` as the sole guard on its one memory-writing primitive; consistent with the hot-path doctrine but the highest-blast-radius `assert` here — on record as a deliberate device-side gap. *(Rejected — intentional. `index()` backs `test()`/`test_and_set()`, the per-pair dedup primitives called in O(pairs) inner loops (edge/face dedup), so it sits squarely in the hot-path-doctrine carve-out: the cheapest correct guard is a debug `assert` that strips to nothing under device `NDEBUG` (zero per-iteration cost) while still trapping an unordered/out-of-range caller on the bench, where the ordering contract is fixed at the source. This is distinct from the cold-site fail-fast policy (arena/OOM/OOB at allocation/construction), which uses an always-on `HS_CHECK` because those sites are not per-iteration hot. The existing code comment already records the gap deliberately; kept as-is.)*
63. ✅ `core/effect_registry.h:82` — `reserve(64)` is a magic literal that silently under-reserves past 64 effects (harmless growth fallback). → Add a threshold comment (or `reserve(HS_EFFECT_COUNT)`). *(Named the literal `constexpr size_t kReserveHint = 64` with an inline "~2x the ~30 shipping effects" note so the threshold is self-documenting at the `reserve()` site; the existing block comment already explains why `HS_EFFECT_COUNT` is unreachable here (include cycle) and why an under-reserve is a harmless growth fallback.)*
64. ✅ `core/presets.h:83` — public mutable `entries`/`current_idx`/`prev_idx` force the runtime OOB trap in `get()`. → Make the indices private with `next()/prev()` the only mutators. *(Made `current_idx`/`prev_idx` private with `next()/prev()` the only mutators, added read-only `current_index()`/`prev_index()` accessors, and added a non-explicit `Presets(std::array<Entry,Size>)` constructor (a private member ends aggregate-ness, so designated init no longer applies). Converted the three designated-init sites — DreamBalls, MeshFeedback, MindSplatter — to the explicit `std::array<…>{{…}}` constructor form; Flyby/Liquid2D's plain brace-init still flows through the constructor unchanged. Dropped the two dead `(array,int)`/`(array,int,int)` CTAD guides (no constructor takes positional indices). `entries` stays public — it is still iterated for noise binding and read by tests. The `get()`/`prev_get()` traps are kept as cheap structural-invariant insurance.)*
65. ✅ `core/canvas.h:39` — the `Effect` ctor runs two full `MAX_W*MAX_H` buffer fills despite `init()` existing to move heavy setup out of the ctor; defensible but worth noting. *(Hoisted the two fills into a `noinline clear_buffers()` helper called from the ctor, so the heavy code is emitted once instead of duplicated into both GCC ctor variants (C1/C2) — the exact code-size concern `init()` exists to address. A full move into `init()` is unsafe and was rejected: the ~27 derived `init()` overrides don't chain to a base `Effect::init()`, so the clear would be silently skipped, leaving uninitialised framebuffers on device (a path CI can't build). Keeping the clear in construction guarantees it runs for every Effect.)*
66. ✅ `core/conway.h:343-349,360` — `relax` is listed once among buffered primitives and once as "no extra index buffers." → Clarify it is a no-scratch primitive in both lists. *(The operator-overview's blanket "all scratch (… plus the per-orbit index/flag buffers)" implied every primitive allocates index buffers; qualified it to "for most primitives … kis and relax allocate none," pointing at the SCRATCH ARENA CONTRACT list that already classifies both as no-extra-index. Comment-only.)*
67. ✅ `core/conway.h:489,497` — inner loops in `kis` reuse the name `i` (no shadow bug, outer loop closed) next to `offset + i`. → Rename to `k` as sibling operators do. *(Renamed both per-face inner loop counters from `i` to `k`, matching the `offset + k` indexing idiom that `dual`/`ambo`/the mesh classifier already use. No behavior change.)*
68. ✅ `core/mesh.h:539` — `classify_faces_by_topology` uses a 32-bit hash directly as the dense topology id with no tiebreak — safe for the fixed solid roster but the WASM mesh-editor is an untrusted boundary where a collision silently merges face classes. → Gate the finer keying behind the trusted/untrusted distinction. *(Resolved-as-documented, comment-only — no behavioral change. The existing note already documented the upgrade path; tightened it to name the one untrusted caller explicitly (`wasm.cpp`'s `Mesh::classifyFaces`) and to bound the actual risk: a collision there is not a memory-safety/invariant breach, only a cosmetic mis-merge of two face palette classes, and the birthday bound needs ~77k distinct face topologies before it is even likely — unreachable for any displayable mesh. The finer-keying upgrade remains gated-to-untrusted IF a future caller ever approaches that density; not implemented now since it would add per-face work for no reachable benefit.)*
69. ✅ `core/solids.h:1301` — `get_by_name` returns a live `cube()` after an "unreachable" `HS_CHECK(false)`; the "no silent wrong solid" guarantee depends on the macro being `[[noreturn]]`. → Use `__builtin_unreachable()`/a `[[noreturn]]` abort instead of a live fallback. *(Replaced the live `finalize_solid(Platonic::cube(...))` fallback with `__builtin_unreachable()`. `check_fail()` is already `[[noreturn]]` and always-on (survives NDEBUG), so the trap always fires; the unreachable removes the only path that could ever return a wrong solid, so the guarantee no longer rests on the macro never being weakened. The unknown-name death case is still covered by `unit_death`.)*
70. ✅ `core/hankin.h:214-219` — `dyn_idx` is captured *before* the matching `emplace_back`, reading as an off-by-one against the `size()-1`-after-push idiom elsewhere. → Add a "pre-push capture is deliberate" note. *(Added a comment: `size()` before `emplace_back` is exactly the index the new dynamic vertex will occupy, so `dyn_idx` names the element created on the next line — not an off-by-one. Comment-only.)*
71. ✅ `core/animation.h:1569` — comment cites "finding 419," violating the project's **no-finding-refs-in-comments** convention. → Drop the number and describe the behavior. (Convention fix.) *(Replaced "see finding 419 on the repeating-spawn slot leak" with a description of the behavior itself: the slot is never reclaimed, so a repeating RandomWalk spawn leaks slots until the pool is exhausted — cancel() each or spawn finite animations.)*
72. ✅ `core/animation.h:982-1003` — `Driver` re-reads `*speed_src_` every frame with no finite guard; a slider momentarily producing NaN permanently poisons the driven param via `wrap_t(NaN)`. → Guard with `std::isfinite`. *(Guarded both the live-bound ctor seed and the per-frame `step()` re-read with `std::isfinite`: a non-finite slider frame keeps the last good speed (ctor keeps the 0.0f seed) instead of poisoning `mutant` (NaN += / wrap_t(NaN) stays NaN forever). The `isfinite` sits on the once-per-frame timeline path, not the pixel loop, so no hot-path cost. Added `test_driver_nan_source_does_not_poison` pinning that a NaN frame is ignored and a recovered slider resumes cleanly.)*
73. ✅ `core/filter.h:1182-1186` — `Screen::Trails::DecayPixel` array is named `ttls_` but holds position+ttl. → Rename to `points_` to match `World::Trails::items_`. *(Renamed the `Screen::Trails` member `ttls_` to `points_` (and the comment reference) — it holds full trail points (x, y, ttl), not just ttls, paralleling `World::Trails::items_`. The per-element `ttl` field is unchanged. Rename-only.)*
74. ✅ `core/transformers.h:36,137` — the template-level `params` member shares its name with `Entity::params`. → Rename to `template_params`/`default_params` to end the shadowing. *(Renamed the `Transformer`-level `params` member to `template_params` (the spawn-copy source) so it no longer shadows the per-slot `Entity::params`. Updated the spawn copy (`e.params = template_params`) and the three live callers — `ChaoticStrings` (`noise_xform`), `IslamicStars` (`ripple_gen`, incl. its `registerParam` slider bindings), and `test_transformers` (`nt`). `Entity::params` and the function-parameter `params` of the free transform functions are unchanged. Rename-only.)*
...
76. ✅ `core/animation.h:2266` — the `Noise` 2²⁴-frame precision-freeze comment assumes `speed==1`. → Note the limit scales inversely with `speed`. *(`noise_transform` samples the time axis at `time*speed`, so each float quantization step is magnified by `speed`; documented that the ~77 h figure is the `speed==1` value and the visible onset scales as ~2²⁴/speed.)*
77. ✅ `core/filter.h:1614-1617` — `sample_bilinear_prev` hand-rolls a negative-x wrap instead of `fast_wrap`. *(Premise corrected: `bx` does NOT exceed the window — the sole producer (Feedback) seam-wraps its deltas to `[-W/2, W/2]`, so `bx = x + ddx ∈ (-W, 2W)` and both taps stay in `fast_wrap`'s `[-W, 2W)`. Rather than document a false rationale, replaced the full-modulo wrap with `fast_wrap` to match the AntiAlias bilerp, the Pixel-domain sinks, and the fail-fast convention — simpler, cheaper, and asserts the producer contract. Bit-identical for in-window coords.)*
...
88. ✅ `effects/GnomonicStars.h:104` — the per-frame `fib_spiral` recompute is justified as "sim-targeted," but the effect is registered for device resolutions like every other. → Enforce the sim-only assumption or tighten the comment. *(Dropped the bogus "sim-targeted, trig is cheap" rationale and removed the recompute outright. The base lattice depends only on `(points, i)` — `eps` is fixed at 0 — so it is identical every frame until "Points" moves. Now cached in a `MAX_POINTS`-Vector buffer carved once in `init()` from the persistent arena (which only the active effect uses, so the ~24 KB is not a standing per-class cost), rebuilt only when `(int)points` changes; the steady-state frame just reads each base point back before warping. Bit-identical output — the effects suite (13495 cases) passes unchanged.)*
...
90. ✅ `effects/Moire.h:62,169` — `density`'s in-class struct default (10.0f) is dead because `init()` overrides it per-resolution. → Note the override on the struct field. *(Documented on the `density` field that `init()` overrides it per-resolution (10 at W<=96, else 45), so the in-class default never reaches a live frame. Comment-only.)*
91. ✅ `effects/Comets.h:258-262` — the class `@note` claims Comets applies `Screen::AntiAlias`, but it uses an empty `Pipeline<W,H>` and deliberately omits AA (the note appears copied from the `ChaoticStrings` sibling). → Correct the per-effect note. *(Rewrote the per-effect difference: this effect AND RingSpin render without AntiAlias; only ChaoticStrings applies it. Cross-referenced the empty-pipeline `filters` member, which already documents the deliberate omission. Comment-only.)*
92. ✅ `effects/Thrusters.h:104,188` — `t_global` is incremented at the end of `draw_frame`, so consumers see last frame's counter (documented invisible offset). → Increment at the top to remove the latent foot-gun. *(Moved `t_global++` from the end of `draw_frame` to its top, before `timeline.step()` runs the only consumers (`on_fire_thruster`, `draw_ring`), so they read the current frame's counter. The counter has no external reference (it only feeds the periodic temporal wave in `ring_fn`), so the one-frame phase shift is invisible; a future top-of-frame consumer can no longer read a stale value. Updated the `on_fire_thruster` comment to match.)*
...
94. ❌ `effects/RingSpin.h:99` — all four rings spawn with the same `Y_AXIS` normal and diverge only over time, so the first frames render coincident (likely intentional, undocumented in an otherwise heavily-commented file). → Add a one-line note. *(Rejected — not a bug. The finding itself concedes the coincident spawn is intentional behavior; the only ask was a documentation note, with no correctness, performance, or invariant issue.)*
95. ❌ `effects/PetalFlow.h:220` — up to 64 rings × W/2 samples is the heaviest curve effect at 288×144; scratch handling is correct. → Profile the per-column budget on device. *(Rejected — not a bug. The finding explicitly states scratch handling is correct; it is a profiling suggestion, not a defect. Device profiling is a separate operational task, not a code-review fix.)*
96. ✅ `effects/MeshFeedback.h:151` — the `is_bound()` guard protects a state `init()` guarantees never occurs; harmless defensive guard. → Removed to simplify (siblings don't guard, and an unbound mesh draws as a no-op anyway).
97. ❌ `effects/DreamBalls.h:247` — the `faces.size()/2 == E` closed-manifold assumption couples the preset table to the edge-count math and traps at `push_back` rather than a clear validation point (fail-fast by design); noted. *(Rejected — not a bug. The trap-at-`push_back` is the codebase's fail-fast doctrine working as intended: a malformed preset crashes loudly on an invariant violation rather than silently degrading. The finding is parenthesized "fail-fast by design" and "noted" — an observation, not a defect.)*
