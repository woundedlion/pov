...
27. ❌ **[mesh] `core/mesh/mesh.h:503`** — `classify_faces_impl` is `always_inline` but is a ~180-line cold routine force-inlined into two `HS_COLD` wrappers, duplicating cold code with no perf benefit on the memory-tight Teensy image. *Rejected:* the `always_inline` is load-bearing for the Teensy budget, and the finding inverts the trade. Force-inlining folds the body into the FLASH-resident (`HS_COLD`/FLASHMEM) wrappers — the "duplication" costs plentiful FLASH (~1.6 MB free). Emitting it once instead lands the standalone function in scarce ITCM: a plain `inline` defaults to ITCM (Teensy FASTRUN), and `HS_COLD`/FLASHMEM is silently dropped on this template (COMDAT section-type conflict). Measured either way, +1.3 KB ITCM crosses phantasm's 32 KB granule cliff (+31 KB padding → RAM1 over budget by 17.8 KB). Keep the `always_inline`.
...
30. ✅ **[math] `core/math/3dmath.h:647-658`** — `Complex::operator/` is not general complex division: it silently caps magnitude at `STEREO_INF` and returns `(0,0)` for 0/0. Correct/intentional for the stereographic call sites, but the operator carries no name signaling it, so any future generic use gets silently clamped quotients. *Resolved:* `operator/` is now ordinary complex division; the projection clamp moved to a named free `project_div()` that `mobius()` calls. Numerically byte-identical to the old operator (WASM/JS parity preserved), no per-pixel regression.

31. ✅ **[animation] `core/animation/sprites.h:169-189`** — `ParticleSystem` exposes `pool`, `active_count`, `friction`, `gravity`, `max_life`, `attractors`, `emitters` as public mutable members, unlike every other animation (all-private); `active_count` is a load-bearing swap-remove invariant an external write would desync. *Resolved:* made `active_count` (the load-bearing counter) private with a read-only `active()` accessor; no production code writes it (spawn/kill go through methods), so encapsulation removes the desync footgun with no setter. `pool` and the genuine tunables stay public.
