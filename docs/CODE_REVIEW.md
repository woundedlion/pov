...
3. **PiP camera renders the identical main-view image** (`Correctness`, `daydream/driver.js:604-606`) — renderPip() copies BOTH camera.position and camera.quaternion into pipCamera each frame, so the picture-in-picture viewport renders a pixel-identical duplicate of the main view. The README states the PiP is "at a fixed orientation" so "the front and back of the sphere are visible simultaneously" — that goal is not met; the PiP shows the same face as the main camera. _Fix:_ Offset the PiP camera to a fixed/opposite orientation (e.g. negate position or apply a fixed azimuth) instead of copying the main pose verbatim.
