...
2. ❌ **Segment clip is sampled one display window early — every clipped frame may paint the wrong half** — `hardware/pov_segmented.h:320-323` — `clip_to_segment(cur, handoff_.window_left() == 0)` runs before `cur->draw_frame()`, but `Canvas`'s ctor blocks on `buffer_free()`, which the ISR only re-satisfies at the next flip; the sampled window is therefore two windows offset from the displayed one against a one-window alternation. A discrete-event model of the four buffer primitives shows frame 1 matching and every subsequent frame mismatching. Fix: spin `while (!cur->buffer_free()) {}` before sampling (moves the wait, does not add one). **Model-derived — confirm on device before landing; no host test covers render→display window phasing.** ❌ Deferred: confirmed on host (a composed model over the real `Canvas`/`EffectHandoff` primitives mismatches 14 of 16 displayed frames, 0 of 16 once the buffer wait precedes the sample), but the proposed fix inverts the parity for a render that crosses a window boundary — `GSReactionDiffusion` spills 999/1024 frames, so it needs the clip sampled at the `Canvas` acquisition seam plus a zero-spill invariant, owned separately.
