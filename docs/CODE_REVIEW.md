...
6. **WASM `setResolution` leaves the engine dead** on an unsupported size (`wasm.cpp:87-127`): nulls `currentEffect` and renders blank with no error to JS. Centralize the supported `(W,H)` dispatch table (currently duplicated 3×) and surface failure.

7. **Geometry edge cases:** `AABB::intersectRay` div-by-zero on axis-aligned rays (`spatial.h:64-97`); `KDTree::nearest` unclamped `k>MAX_K` silently drops results (`spatial.h:152-209`); `OrientationTrail::get` has contradictory oldest/newest ordering docs (`animation.h:230-233`) that can reverse motion-blur direction.
...
- **High** | `Arena::allocate` returns `nullptr` on OOM but Conway/Hankin/HalfEdge scratch allocations deref unconditionally (`conway.h:206-212`, `mesh.h:135`, `hankin.h:90-95`). Per fail-fast, `allocate` should `HS_CHECK` non-null internally (over-allocation = bug → trap) rather than returning null for callers to ignore.
- **High** | `ArenaVector` capacity guards are stripped `assert`s on device (`memory.h:250,261,269`) → silent OOB. Replace with `HS_CHECK` so a capacity overrun traps in the release build instead of corrupting the arena.
- **Medium** | `spatial.h:64-97` ray-AABB div-by-zero; `spatial.h:152-209` unclamped `k` (a `k>MAX_K` caller is a bug → `HS_CHECK`, not silent drop); `animation.h:230-233` contradictory trail-ordering docs; `conway.h:78-104` `vertex_orbit` assumes manifold closure (add a hard `count<=I` loop bound independent of asserts).
...
- **High** | `Voronoi.h:71,123` OOB + div-by-zero.
- **Medium** | `GSReactionDiffusion.h:128` NaN from `wb/tw` (BZ guards this, GS does not — same algorithm, divergent robustness).
...
- **High** | `wasm.cpp:87-127` — `setResolution` on an unsupported size leaves the engine null/dead with no error to JS.
