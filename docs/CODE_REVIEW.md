...
1. **Clamp `lerp_oklch` inputs / outputs** (`color.h:509-526`). Extrapolated `t` (reachable via unbounded lerp amounts) drives L or C negative → wrong, often near-black, colors. Clamp `t` at entry or L/C at exit.
