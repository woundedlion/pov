...
14. ❌ [Low] **daydream workers — no test asserts the overrun re-blit produces a non-mixed frame.** This gap is exactly why Fix #3 slipped through; add the assertion alongside the fix. _Rejected: depends on Fix #3, which is not in this batch; asserting a non-mixed frame would fail against the current, deliberate generation-mixing behavior._
