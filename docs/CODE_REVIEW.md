...
18. ❌ [Low] **Geometry — `Style` copy silently drops the `noise` binding + hue cache.** `Style` is trivially assignable, but a full-struct copy (`style = Style::Churn()`, `Presets::apply`) resets `noise` to `nullptr` (degrading `noise_warp` → identity) until `sync_noise()`/`sync_hue()` re-run — an unwarped, un-hue-shifted frame with no error (`styles.h:94`). Rejected: the trivial-copy semantics are intentional and required by `Presets<>`/`lerp` (preset value copy); a custom copy that preserved `noise`/hue would break that contract. The struct already documents the re-sync requirement, and every copy site re-binds noise and calls `sync_*` on the next frame.
...
28. ❌ [Low] **Render — `SDFShape` concept under-specifies the scan contract.** It requires only `is_solid`/`thickness` but the rasterizer also needs `distance<>()`, `get_vertical_bounds<H>()`, `get_horizontal_intervals<W,H>()` (`sdf.h:307`); a shape missing those fails deep in template instantiation. Widen the concept. Rejected: the scan members are templated on canvas `W`/`H`, which are unavailable where `static_assert(SDFShape<…>)` fires (the combinator's class instantiation, e.g. `Union<A,B>` — `W`/`H` arrive only at the later `get_horizontal_intervals<W,H>()` call). So the concept structurally cannot name them. It is also correctly minimal for `AngularRepeat`, whose child needs only `distance()`; widening would over-constrain that valid case.
...
30. ❌ [Low] **Animation — MeshMorph per-frame `slerp` can approach the antipodal degenerate** on a sparse-source nearest match; the dot-max match minimizes but does not bound the angle (`mesh.h:135`). Low likelihood; the exact-antipode gate handles only the exact case. Rejected: the premise misreads `slerp` — its antipodal branch fires for the whole `θ > π − TOLERANCE` band (`3dmath.h:1227`), not just the exact antipode, and rotates about an arbitrary perpendicular axis for a monotone half-turn landing within `TOLERANCE` of `v2`. A near-antipodal match is therefore already handled; there is no unbounded degenerate.
31. ❌ [Low] **Animation — `add_get` defaults `pin = true`.** Pinning a *finite* animation is caught only at completion, not at add, and the `true` default silently marks events non-relocatable (`timeline.h:178`). Assert `!is_finite()` at add for pinned events. Rejected: the `pinned => infinite` contract is already enforced fail-fast on both exit paths a pinned finite event can take — relocation (`TimelineEvent::move_into` HS_CHECK, death-tested by `case_timeline_handled_relocation`) and natural completion (step()'s completion branch, death-tested by `case_timeline_handled_completion`, which pins a 1-frame Transition on purpose). An add-time assert is redundant and would make that completion-branch death case unreachable, removing coverage of the trap it exists to prove.
...
33. ❌ [Low] **daydream — `gui.js` `applyOnLoad` latch re-fires on every later `onChange`** registration, diverging from lil-gui's contract that registration does not fire the handler (`gui.js:176-191`); a footgun for the tool pages that reuse the wrapper. Rejected: the per-registration replay is deliberate and documented at the site — it fans the deep-linked side effect out to every `onChange` consumer, not just the first, so a second fan-out handler isn't skipped by a shared latch. All handlers are registered synchronously at construction; no caller registers post-load. The load-time replay semantics are pinned by `gui.test.js`, so the divergence is intentional.
...
43. ✅ [Low] **Effects (group 1) — `Params`/sub-struct visibility and field order inconsistent.** Public-at-top vs private-at-bottom varies; HopfFibration's struct field order differs from its registration order (`HopfFibration.h:95-101` vs `:39-43`). Cosmetic but invites a wrong "fix." Unified all effect classes to the convention already used by the majority — `public:` first, then `private:` with `params` a private member at the bottom — by relocating the public/mid `Params` in ChaoticStrings, GnomonicStars, PetalFlow, SplineFlow, HopfFibration, DreamBalls, and Voronoi (which gained a `private:` section); reordered HopfFibration's fields to match registration order; and routed the now-private `params` in the DreamBalls/HopfFibration white-box tests through new `WhiteBox` accessors, per the existing GS/BZ pattern.
44. ❌ [Low] **Effects (group 1) — DistortedRing's amplitude wave ignores the pause gate** (`DistortedRing.h:57`); "Pause Animation" cannot hold the rings still as it can in every sibling. Gate the mutation or document the absence. Rejected: "Pause Animation" is scoped to pausing parameter (slider-backed) animation so the user can take a slider over; the amplitude wave is structural visual motion, not a registered param, so leaving it ungated is correct.
45. ✅ [Low] **Effects (group 1) — Voronoi dead `sharpness > 0` guard** (`Voronoi.h:105`): the slider floor is `1.0`, so the branch is unreachable. Lower the floor or drop the comparison. Lowered the slider floor to `0.0` so `sharpness == 0` reaches the guard and disables the cross-cell blend.
46. ✅ [Low] **Effects (group 1) — Moire dead struct-default silently duplicated** with the `W<=96` branch value (`Moire.h:169`,`:56`); if the resolution branch is removed, the stale default becomes live with no warning. Replaced the `10.0f` default with `0.0f`, below the `[1,100]` slider floor, so dropping the per-resolution assignment now traps at `register_param` instead of silently going live.
47. ✅ [Low] **Effects (group 2) — MindSplatter vestigial `opacity` parameter** always passed `1.0` (`MindSplatter.h:247`). Per the repo's remove-dead-code-over-documenting rule, drop the parameter. Kept the `opacity = 1.0f` default parameter and removed the redundant explicit `1.0f` at the sole caller.
...
51. ❌ [Low] **Animation — motion-blur virtuals leak into the universal interface.** `collapse_orientation`/`orientation_id` sit on `IAnimation` but only ~2 of ~20 families implement them (`animation.h:87-96`). Isolate via an `OrientedAnimation` mixin. — Rejected: the two virtuals are load-bearing on `IAnimation` because `Timeline`'s per-frame collapse loop dispatches through `IAnimation*`; a mixin can only relocate the (differing) override bodies, not remove the interface surface the finding objects to, and doing so needs a disproportionate `Timeline` rework (RTTI/registry) for a Low item.
52. ❌ [Low] **Animation — single-slot `.then()` traps a second chain** (`HS_CHECK(!post)`, `animation.h:178`) rather than composing; limits sequencing expressiveness. — Rejected: composing a second callback overflows the fixed `Fn<void(),24>` inline slot, and a second post slot enlarges every animation against the 112 B device inline-storage budget (`LARGEST_CONCRETE_ANIM_SIZE` static_assert); the single-slot trap is a deliberate design that surfaces accidental double-registration.
...
55. ❌ [Low] **Mesh — one-shot `hankin()` reverses arena polarity** so `target` must hold `max(compile-scratch, output)` (`hankin.h:364-389`), unlike every other operator; a caller sizing `target` for the output alone under-provisions and traps deep inside `compile_hankin`. Assert the headroom at the call boundary. Rejected: the compile-scratch peak is a data-dependent function of the input mesh's half-edge count, so a correct boundary "headroom" assert would have to duplicate `compile_hankin`'s internal sizing formula (fragile, rot-prone); the polarity/sizing contract is already documented in the `@param` doc, and the Arena over-allocation trap is the codebase's intended always-on fail-fast guard at the canonical allocation point.
56. ❌ [Low] **Mesh — composition-polarity input/output arena sharing has no static capacity guard** (`solids.h:218`, `conway.h:324`); a new recipe combining a composed op with a high-expansion successor could overflow only at runtime. Rejected: Conway mesh expansion is data-dependent (per-op, per-seed, per-chain) with no compile-time expression to bound, so a static capacity guard is infeasible; the polarity contract is already documented at both cited sites and the runtime Arena over-allocation trap is the intended always-on guard.
57. ❌ [Low] **Geometry — `Style` downsample lerp snaps to the denser grid at t=0.5**, spiking coarse-grid scratch demand (~16× for 8→2) mid-transition (`styles.h:115`,`:148`); an effect sized for the coarser endpoint traps. Surface the peak requirement at the provisioning site. Rejected: unreachable — every `Style` preset leaves `downsample` at the default 4, so the snap is always 4→4 and stays on the cacheable zero-scratch path; the per-DS scratch peak is already documented at the `downsample` field, and the `scratch_arena_a` over-allocation trap is the intended always-on guard (same basis as findings 55/56).
...
60. ❌ [Low] **daydream — `statsGroup` caches element handles (including nulls) once** for the page lifetime (`driver.js:784-793`); brittle if the stats DOM is built after first paint. Rejected: all ten stat elements are static markup in `index.html`, present before the first frame, so the "built after first paint" case does not occur; each use already null-guards with `if (el)`, and re-querying on the per-frame `updateStats` hot path would be an unwarranted cost.
...
62. ❌ [Low] **daydream workers — per-frame `Uint16Array` transfer-buffer allocation** (`segment_worker.js:191`,`:236`); unavoidable without a controller→worker return channel, but a per-worker per-frame GC source worth eliminating if worker churn shows in profiling. Rejected: the finding is self-gated on profiling evidence ("if worker churn shows in profiling") that does not exist, and the only real fix — a controller→worker recycle channel plus worker free-pool — adds a permanent hot-path protocol invariant (recycle-before-render ordering, size-discard, neuter guard) and a `PROTOCOL_VERSION` bump, disproportionate to a [Low] on the diagnostic segmented-preview path.
...
65. ❌ [Low] **Targets — `capture_screenshots.mjs` brittly scrapes daydream's `<option>` text and `?effect=` URL scheme** with no cross-repo version pin (`capture_screenshots.mjs:129`,`:182`); a daydream UI change silently degrades the gallery. Fails safe with a loud summary, but an asserted contract would harden it. Rejected: the script already fails safe with a loud end-of-run summary (per-effect `wrongRes`/`blanks`/`failures` plus the resolutions-unresolved banner), and it is a dev-only gallery tool; a hard assert would trade a benign silent-degrade for a hard crash on any daydream UI tweak, and there is no shared cross-repo version handle to pin against.
