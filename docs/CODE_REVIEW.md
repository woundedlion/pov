...
3. ❌ **`slerp(Vector)` antipodal branch tests a threshold finer than `fast_acos`'s own error.** Not viable as proposed: the exact-equivalent raw-dot threshold `-1 + TOLERANCE²/2 ≈ -1 + 5e-9` is below `ULP(1) ≈ 1.2e-7`, so it rounds to exactly `-1.0f` in float32 and the antipodal branch never fires on exact antipodes — dropping them into the general path where `‖s1·v1 + s2·v2‖ = 0` and `normalize()` traps. A representable `k` (e.g. `TOLERANCE`) would instead break the endpoint contract for near-but-not-exactly-antipodal inputs. The original `fast_acos(d) > PI_F - TOLERANCE` is correct here (`fast_acos(-1) = π` exactly; the error envelope is negligible at the endpoint) and stays finite/unit-length. (`core/3dmath.h:1225-1232`)
...
30. ❌ (deriving from the shared list yields only the case-*function* count — 5 for the clamp list vs its 26 assertions, a 5× weaker guard — and `h_offset_renorm` has no shared list at all; the independent hand-set floor is the correct anti-drift mechanism precisely because it does not track the code under test, and the "bump when adding" notes are already present) **`kMinAssertions` floors in the auxiliary TUs require manual bumping.** Hand-counted magic numbers (26, 108) that drift unless someone remembers the manual step; the guard only catches wholesale gutting. *Fix:* derive the floor from the shared case list where feasible, or at least keep the "bump when adding" note tight. (`tests/fastmath_clamp_check.cpp:24-31`, `tests/h_offset_renorm_check.cpp`)
