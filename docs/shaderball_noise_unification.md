...
| Curl Flow warp | Selected projection's plane | A central-difference gradient of one scalar field is rotated 90 degrees and clamped to four units per component. Simplex has a transformed-coordinate fast path. Euler or midpoint integration follows the planar field. | Same stage phase as Vector Noise. | Has the same chart seams. Stability requires `scale * abs(strength) * 4 / intervals <= 0.5`. |
