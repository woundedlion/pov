# KaleidoscopeStainedGlass on-device profile — Teensy 4.0, segmented mode (2026-08-16, **selective -O3**)

Point-in-time snapshot (regenerate with `just profile KaleidoscopeStainedGlass`).
Raw capture: `build/prof/kaleidoscopestainedglass_ship.log`.
...
| Effect | KaleidoscopeStainedGlass 288×144, single-entry playlist, tip `69d4751cc077` |
...
A display window is 62.5 ms. KaleidoscopeStainedGlass is a strobe effect rendering one
...
Phase schedule: KaleidoscopeStainedGlass has a single preset and one steady regime — the
...
`HS_PROFILE_WINDOW=32`; `just profile KaleidoscopeStainedGlass` performs the locked build,
