# HexWave on-device profile — Teensy 4.0, segmented mode (2026-08-16, **selective -O3**)

Point-in-time snapshot (regenerate with `just profile HexWave`).
Raw capture: `build/prof/hexwave_ship.log`.
...
| Effect | HexWave 288×144, single-entry playlist, tip `69d4751cc077` |
| Method | `HS_PROFILE` cycle scopes, window = 32 frames, 70 s capture |
| Reproduce | `bash tools/profile_one.sh HexWave profile 70 32` |
...
A display window is 62.5 ms. HexWave is a strobe effect rendering one
...
Phase schedule: HexWave has a single preset and one steady regime — the
...
`targets/Profile/Profile.ino` with `HS_PROFILE_TARGET=HexWave` and
`HS_PROFILE_WINDOW=32`; `just profile HexWave` performs the locked build,
