# ContourLattice on-device profile — Teensy 4.0, segmented mode (2026-08-16, **selective -O3**)

Point-in-time snapshot (regenerate with `just profile ContourLattice`).
Raw capture: `build/prof/contourlattice_ship.log`.
...
| Effect | ContourLattice 288×144, single-entry playlist, tip `69d4751cc077` |
| Method | `HS_PROFILE` cycle scopes, window = 32 frames, 70 s capture |
| Reproduce | `bash tools/profile_one.sh ContourLattice profile 70 32` |
...
A display window is 62.5 ms. ContourLattice is a strobe effect rendering one
...
Phase schedule: ContourLattice has a single preset and one steady regime — the
...
`targets/Profile/Profile.ino` with `HS_PROFILE_TARGET=ContourLattice` and
`HS_PROFILE_WINDOW=32`; `just profile ContourLattice` performs the locked build,
