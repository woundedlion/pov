# KaleidoWave on-device profile — Teensy 4.0, segmented mode (2026-08-16, **selective -O3**)

Point-in-time snapshot (regenerate with `just profile KaleidoWave`).
Raw capture: `build/prof/kaleidowave_ship.log`.
...
| Effect | KaleidoWave 288×144, single-entry playlist, tip `69d4751cc077` |
| Method | `HS_PROFILE` cycle scopes, window = 32 frames, 70 s capture |
| Reproduce | `bash tools/profile_one.sh KaleidoWave profile 70 32` |

Image size:

```text
FLASH: code:67,000, data:149,460, headers:8,820
...
Exactness cross-check: window frames 641-672 root counter cycles ÷ 600 MHz
match the measured wall sum within **2.6 ppm**. Validation reports
...
**27.89 ms** at frames 769-800, spilled **0/1,088 frames** (0%).


A display window is 62.5 ms. KaleidoWave is a strobe effect rendering one
...
of the pass keeps **34.61 ms** of the window in hand, so the effect holds
16 fps for the whole capture. `canvas_buffer_wait` is the round-up idle to the
next display flip, by design.

## Phase-by-phase readout

Phase schedule: KaleidoWave has a single preset and one steady regime — the
...
frame                  62.44 ms  37.47 Mcyc  100%
  fx_shader_draw       21.55 ms  12.93 Mcyc   34%
  fx_prepare_frame      3.86 ms   2.32 Mcyc    6%
  fx_advance            2.10 ms   1.26 Mcyc    3%
  fx_timeline_step      56.7 us  34.05 kcyc    0%
  canvas_clear          86.1 us  51.64 kcyc    0%
  canvas_buffer_wait   34.79 ms  20.87 Mcyc   55%
```

Wall min/avg/max = 62.07/62.44/62.87 ms. `fx_shader_draw` is 34% of the
frame at 21.55 ms; the whole render is 27.65 ms and the remaining
34.79 ms (55%) is `canvas_buffer_wait` idle. `fx_prepare_frame` costs
3.86 ms and `fx_advance` 2.10 ms — the runtime advance, the spatial-frame
...
The dominant scope spends 1,247 cycles per quadrant sample over the
...
```text
isr_wake        1152/frame  min/avg/max 0.68/1.98/15.83 us  cpu 3.65%
isr_pack         144/frame  min/avg/max 6.29/6.97/9.44 us  cpu 1.60%
isr_dma_submit   144/frame  min/avg/max 0.63/0.93/4.73 us  cpu 0.21%
```
...
- Total ISR CPU share is 5.46%, already absorbed by the render
  counters. With 34.61 ms of margin at the worst frame, no speedup is
  required for cadence.

## Summary ranking

1. `fx_shader_draw` — 34% of the frame, 21.55 ms: the inlined pullback shade
   over 10,368 samples. Stereographic projection through a `Kaleidoscope` lens,
   an inner `MirrorTile` planar warp, and a `TwinWave` source under a triadic
   noise-hue palette.
2. `fx_advance` — 3% of the frame, 2.10 ms: preset transition bookkeeping and
   the generated-palette cycler step.
3. `fx_prepare_frame` — 6% of the frame, 3.86 ms: per-frame snapshot of the
...
`targets/Profile/Profile.ino` with `HS_PROFILE_TARGET=KaleidoWave` and
`HS_PROFILE_WINDOW=32`; `just profile KaleidoWave` performs the locked build,
