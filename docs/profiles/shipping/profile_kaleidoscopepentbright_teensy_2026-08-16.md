# KaleidoscopePentBright on-device profile — Teensy 4.0, segmented mode (2026-08-16, **selective -O3**)

Point-in-time snapshot (regenerate with `just profile KaleidoscopePentBright`).
Raw capture: `build/prof/kaleidoscopepentbright_ship.log`.
...
| Effect | KaleidoscopePentBright 288×144, single-entry playlist, tip `69d4751cc077` |
...
A display window is 62.5 ms. KaleidoscopePentBright is a strobe effect rendering one
...
Phase schedule: KaleidoscopePentBright has a single preset and one steady regime — the
...
`HS_PROFILE_WINDOW=32`; `just profile KaleidoscopePentBright` performs the locked build,
