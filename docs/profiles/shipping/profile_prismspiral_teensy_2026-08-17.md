# PrismSpiral on-device profile — Teensy 4.0, segmented mode (2026-08-17, **selective -O3**)

Point-in-time snapshot (regenerate with `just profile PrismSpiral`).
Raw capture: `build/prof/prismspiral_ship.log`. First profile of this effect;
no prior report.
...
| Effect | PrismSpiral 288×144, single-entry playlist, tip `fd982e2e` (branch `prismspiral/promote`) |
...
| Reproduce | `just profile PrismSpiral` |
...
**4.8 ppm** (`tools/parse_profile.py build/prof/prismspiral_ship.log validate`).
...
Phase schedule: 600-frame hold on `prism-spiral` → 480-frame parameter
crossfade → 600-frame hold on `prism-spiral-2` → crossfade back (wrap
...
### prism-spiral-2 hold (window frames 1553–1568, worst of the capture)
...
### Crossfade toward prism-spiral (window frames 1841–1856)
...
|---|---|--:|--:|--:|--:|
| 2 | prism-spiral-2 | full quadrant | 36.88 | 40.52 | 16 |
| 1 | prism-spiral | full quadrant | 35.01 | 39.63 | 16 |
...
1. `fx_shader_draw` — 51–59% of the frame, 35.0–36.9 ms clean-hold (40.5 ms
   ...
   `prism-spiral-2` costs ~1.9 ms/f more than `prism-spiral`.
...
- Captured from worktree branch `prismspiral/promote` (`fd982e2e`), not yet
  landed on master at capture time; the tree was clean at the profiled tip.

## Harness

`targets/Profile/Profile.ino` + `HS_PROFILE_TARGET=PrismSpiral`,
`HS_PROFILE_WINDOW=16`; `just profile PrismSpiral` = build + flash + capture.
