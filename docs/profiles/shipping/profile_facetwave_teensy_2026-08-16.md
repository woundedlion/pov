# FacetWave on-device profile — Teensy 4.0, segmented mode (2026-08-16, **selective -O3**)

Point-in-time snapshot (regenerate with `just profile FacetWave`).
Raw capture: `build/prof/facetwave_ship.log`.
...
| Effect | FacetWave 288×144, single-entry playlist, tip `69d4751cc077` |
| Method | `HS_PROFILE` cycle scopes, window = 32 frames, 70 s capture |
| Reproduce | `bash tools/profile_one.sh FacetWave profile 70 32` |
...
A display window is 62.5 ms. FacetWave is a strobe effect rendering one
...
Phase schedule: FacetWave has a single preset and one steady regime — the
...
`targets/Profile/Profile.ino` with `HS_PROFILE_TARGET=FacetWave` and
`HS_PROFILE_WINDOW=32`; `just profile FacetWave` performs the locked build,
