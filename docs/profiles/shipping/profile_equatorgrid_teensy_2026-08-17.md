# EquatorGrid on-device profile — Teensy 4.0, segmented mode (2026-08-17, **selective -O3**)

Point-in-time snapshot (regenerate with `just profile EquatorGrid`).
Raw capture: `build/prof/equatorgrid_ship.log`.
Replaces `profile_equatorgrid_teensy_2026-08-16.md`: re-captured after the
...
CurlLattice 16 ms/frame is absent here — but this look shades **1.5% slower**
...
| Effect | EquatorGrid 288×144, single-entry playlist, tip `1e717361` |
...
CurlLattice, which the same tip shades 11% *faster*. Both directions have the
same cause: the instance pipeline hoists per-frame state out of the per-sample
path. CurlLattice had expensive hoistable work — resolving the curl loop point
once per frame instead of 10,368 times — so it won big. EquatorGrid's stages
...
- Unlike CurlLattice, this look's `shade` and `Source::grid` are out-of-line at
