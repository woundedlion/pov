# LatticeMelt on-device profile — Teensy 4.0, segmented mode (2026-08-18, **selective -O3**)

Point-in-time snapshot (regenerate with `just profile LatticeMelt`).
Raw capture: `build/prof/latticemelt_ship.log`.
Replaces `profile_latticemelt_teensy_2026-08-17.md`: re-captured after the
...
| Effect | LatticeMelt 288×144, single-entry playlist, tip `7da9afbdc` |
...
A display window is 62.5 ms. LatticeMelt is a strobe effect rendering one
