# KaleidoscopeSmooth on-device profile — Teensy 4.0, segmented mode (2026-08-16, **selective -O3**)

Point-in-time snapshot (regenerate with `just profile KaleidoscopeSmooth`).
Raw capture: `build/prof/kaleidoscopesmooth_ship.log`.
...
| Effect | KaleidoscopeSmooth 288×144, single-entry playlist, tip `69d4751cc077` |
...
A display window is 62.5 ms. KaleidoscopeSmooth is a strobe effect rendering one
...
`HS_PROFILE_WINDOW=16`; `just profile KaleidoscopeSmooth` performs the locked build,
