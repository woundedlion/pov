# GlitchGrid on-device profile — Teensy 4.0, segmented mode (2026-08-16, **selective -O3**)

Point-in-time snapshot (regenerate with `just profile GlitchGrid`).
Raw capture: `build/prof/glitchgrid_ship.log`.
...
| Effect | GlitchGrid 288×144, single-entry playlist, tip `69d4751cc077` |
| Method | `HS_PROFILE` cycle scopes, window = 32 frames, 70 s capture |
| Reproduce | `bash tools/profile_one.sh GlitchGrid profile 70 32` |
...
A display window is 62.5 ms. GlitchGrid is a strobe effect rendering one
...
Phase schedule: GlitchGrid has a single preset and one steady regime — the
...
`targets/Profile/Profile.ino` with `HS_PROFILE_TARGET=GlitchGrid` and
`HS_PROFILE_WINDOW=32`; `just profile GlitchGrid` performs the locked build,
